...
require_login();
require_once __DIR__ . '/current-account-types.php';
$accountTypeOptions = current_account_types(is_admin());
require __DIR__ . '/patient-layout.php';
$accountWindowQuery = (string)($_GET['_vox_window'] ?? $_POST['_vox_window'] ?? '') === '1' ? '&_vox_window=1' : '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $checkCompany = $pdo->prepare("SELECT code FROM current_accounts WHERE id=?");
        $checkCompany->execute([(int)($_POST['id'] ?? 0)]);
        if ($checkCompany->fetchColumn() === 'CR-00') {
            http_response_code(422);
            exit('CR-00 işletme hesabı silinemez.');
        }
    ...
    $existingTypeAccount = null;
    if ($action === 'update') {
        $typeStatement = $pdo->prepare('SELECT code,account_type FROM current_accounts WHERE id=?');
        $typeStatement->execute([(int)($_POST['id'] ?? 0)]);
        $existingTypeAccount = $typeStatement->fetch() ?: null;
    }
    try {
        $data['account_type'] = current_account_resolve_type($data['account_type'], is_admin(), $existingTypeAccount);
    ...
    if ($action === 'update') {
        ...
        if ($data['code'] === '' || $data['title'] === '' || !in_array($data['account_type'], ['customer','supplier','both','institution','business'], true)) {
        ...
        header('Location: ' . url('current-accounts.php?edit=' . $accountId . '&updated=1' . $accountWindowQuery));
        exit;
    }
    try {
        $data['code'] = next_current_account_code($pdo);
        if ($data['code'] === '' || $data['title'] === '' || !in_array($data['account_type'], ['customer','supplier','both','institution','business'], true)) throw new RuntimeException('Cari kodu, cari unvanı ve cari tipi zorunludur.');
        ...
        $savedAccountId = (int)$pdo->lastInsertId();
        header('Location: ' . url('current-accounts.php?edit=' . $savedAccountId . '&saved=1' . $accountWindowQuery));
        exit;
...
if (isset($_GET['saved'])) $message = 'Cari kart kaydedildi.';
...
?>
<link rel="stylesheet" href="<?=url('assets/classic-current-accounts.css?v=20260824-4')?>">
...
<style>.account-documents-button{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:43px!important;min-width:43px!important;height:43px!important;margin:24px 0 0 8px!important;padding:0!important;border:0!important;border-radius:7px!important;background:#7367f0!important;color:#fff!important;vertical-align:top!important;cursor:pointer}.account-documents-button .ti{font-size:20px!important}</style>
...
<script>(()=>{const button=document.querySelector('.account-documents-button');if(!button)return;button.addEventListener('click',()=>{const accountId=button.dataset.accountId;if(accountId)location.href=<?=json_encode(url('current-account-documents.php?id='))?>+encodeURIComponent(accountId);});})();</script>
<style>body#vox-app .account-page .account-type-hidden{display:none!important}</style>
<script>
(()=>{
  const type=document.querySelector('select[name="account_type"]');
  if(!type)return;
  const storedType=<?=json_encode((string)($editingAccount['account_type']??''))?>;
  const refresh=()=>{
    const selected=type.disabled?storedType:type.value;
    const hidden=selected==='institution'||selected==='business';
    [document.querySelector('.technical-service-options'),document.querySelector('textarea[name="shipping_address"]')?.closest('label')].forEach(field=>{
      if(field){field.classList.toggle('account-type-hidden',hidden);field.hidden=hidden;}
    });
  };
