...
<main class="patient-container unit-patients-page"><section class="unit-patients-card"><header class="unit-patients-head"><div><h2>Hasta Bilgileri</h2><p><?=e((string)$unit['code'])?> — <?=e($unitName)?></p></div><a href="<?=e(url('units.php'))?>">Ünitelere Dön</a></header><table class="unit-patients-table"><thead><tr><th>AD SOYAD</th><th>SATIŞ FİYATI</th><th>HİZMET ADI</th><th>SONUÇ</th><th>ŞUBE</th><th>İLGİLENEN KİŞİ</th></tr></thead><tbody><?php foreach ($patients as $patient): ?><tr data-patient-url="<?=e(url('patient-form.php?id=' . (int)$patient['id'] . '&return=' . rawurlencode('unit-patients.php?unit_id=' . $unitId)))?>"><td><b><?=e((string)$patient['full_name'])?></b></td><td><?=($salesTotals[(int)$patient['id']] ?? 0) > 0 ? e(number_format($salesTotals[(int)$patient['id']], 2, ',', '.')) . ' TL' : '—'?></td><td><?=e((string)($patient['service_name'] ?? '')) ?: '—'?></td><td><?=e((string)($patient['result_name'] ?? '')) ?: '—'?></td><td><?=e((string)($patient['branch_name'] ?? ''))?></td><td><?=e((string)($patient['contact_person'] ?? ''))?></td></tr><?php endforeach; if (!$patients): ?><tr><td colspan="6" class="unit-patients-empty">Bu üniteye atanmış hasta kaydı bulunmuyor.</td></tr><?php endif; ?></tbody></table></section></main>
