...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ((string)($_POST['action'] ?? '') === 'save_delivery_date') {
        ...
        if (preg_match('/^(\d{2})\.(\d{2})\.(\d{4})$/', $deliveryDate, $parts)) $deliveryDate = $parts[3] . '-' . $parts[2] . '-' . $parts[1];
        if ($deliveryDate !== '') {
            $parsedDate = DateTimeImmutable::createFromFormat('!Y-m-d', $deliveryDate);
            if (!$parsedDate || $parsedDate->format('Y-m-d') !== $deliveryDate) {
                http_response_code(422);
                echo json_encode(['success' => false, 'message' => 'Teslim tarihini gg.aa.yyyy biçiminde girin.']);
                exit;
            }
        }
...
?>
<style>
.sgk-page{width:100%!important;max-width:1220px!important;margin:0 auto!important;padding:96px 20px 48px!important}.sgk-list{width:calc(100% - 64px);margin:28px 32px 48px;border:1px solid var(--line);border-radius:8px;background:var(--card);overflow:hidden}.sgk-list-head{display:flex;align-items:center;justify-content:space-between;min-height:70px;padding:0 24px;border-bottom:1px solid var(--line)}.sgk-list-head h2{margin:0;font-size:20px;font-weight:500}.sgk-list-head p{margin:4px 0 0;color:var(--muted);font-size:13px}.sgk-list table{width:100%;border-collapse:collapse}.sgk-list th,.sgk-list td{padding:13px 18px;border-bottom:1px solid var(--line);text-align:left;white-space:nowrap}.sgk-list th{font-size:12px;color:var(--muted)}.sgk-list tbody tr[data-edit-url]{cursor:pointer}.sgk-list tbody tr[data-edit-url]:hover{background:rgba(32,164,71,.05)}.sgk-list td.money{color:#19a94b;font-weight:700}.sgk-list .delivery-date{width:130px;min-height:36px;padding:6px 8px;border:1px solid #d5d3de;border-radius:6px;background:var(--card);color:var(--text);font:inherit}.delivery-date-picker{width:30px;height:36px;margin-left:0;padding:0;border:0;border-radius:0;background:transparent;color:#343348;cursor:pointer;font-size:16px}.delivery-date-native{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.sgk-list td.collection{text-align:center}.sgk-list td.collection input{width:18px;height:18px;accent-color:#19a94b;cursor:pointer}.sgk-empty{text-align:center;color:var(--muted)}@media(max-width:720px){.sgk-page{padding:92px 12px 30px!important}.sgk-list{width:auto;margin:20px 12px 30px;overflow:auto}.sgk-list table{min-width:880px}.sgk-list-head{padding:0 16px}}
</style>
<main class="patient-container sgk-page"><section class="sgk-list"><div class="sgk-list-head"><div><h2>SGK Listesi</h2><p><?=count($sales)?> kayıt</p></div></div><table><thead><tr><th>SATIŞ TRH</th><th>TESLİM TRH</th><th>FATURA NO</th><th>AD SOYAD</th><th>SGK DESTEĞİ</th><th>VADE TRH</th><th>TAHSİLAT</th></tr></thead><tbody><?php foreach($sales as $sale):?><tr data-edit-url="<?=e(url('patient-followup.php?id=' . (int)$sale['patient_id'] . '&edit=' . (int)$sale['id'] . '&open_sales_details=1'))?>"><td><?=e(format_date_tr((string)$sale['service_date']))?></td><td><input class="delivery-date" type="text" inputmode="numeric" maxlength="10" placeholder="gg.aa.yyyy" value="<?=e($sale['delivery_date'] ? format_date_tr($sale['delivery_date']) : '')?>" data-service-id="<?=(int)$sale['id']?>" data-sale-date="<?=e((string)$sale['service_date'])?>" data-saved-value="<?=e($sale['delivery_date'] ? format_date_tr($sale['delivery_date']) : '')?>" aria-label="Teslim tarihi"></td><td><?=e($sale['invoice_no'] ?: '—')?></td><td><?=e($sale['full_name'])?></td><td class="money"><?=$sale['sgk_support'] > 0 ? number_format($sale['sgk_support'],2,',','.') . ' ₺' : '—'?></td><td data-due-date><?=e($sale['due_date'] ? format_date_tr($sale['due_date']) : '—')?></td><td class="collection"><input type="checkbox" aria-label="Tahsilat tamamlandı" <?=($sale['sale_total'] > 0 && $sale['collection'] >= $sale['sale_total'] - .009) ? 'checked' : ''?>></td></tr><?php endforeach;if(!$sales):?><tr><td colspan="7" class="sgk-empty">Henüz satış kaydı bulunmuyor.</td></tr><?php endif?></tbody></table></section></main>
<script>const deliveryIso=value=>{const match=String(value||'').match(/^(\d{2})\.(\d{2})\.(\d{4})$/);return match?`${match[3]}-${match[2]}-${match[1]}`:''};document.querySelectorAll('.delivery-date').forEach(input=>{input.addEventListener('input',()=>{const digits=input.value.replace(/\D/g,'').slice(0,8);input.value=digits.replace(/(\d{2})(\d)/,'$1.$2').replace(/(\d{2}\.\d{2})(\d)/,'$1.$2');});input.addEventListener('change',async()=>{const deliveryDate=deliveryIso(input.value);if(input.value&&!deliveryDate){alert('Teslim tarihini gg.aa.yyyy biçiminde girin.');input.value=input.dataset.savedValue||'';return;}if(deliveryDate&&input.dataset.saleDate&&deliveryDate<input.dataset.saleDate){alert('Teslim tarihi satış tarihinden küçük olamaz.');input.value=input.dataset.savedValue||'';return;}const data=new FormData();data.set('csrf',<?=json_encode(csrf())?>);data.set('action','save_delivery_date');data.set('service_id',input.dataset.serviceId||'');data.set('delivery_date',deliveryDate);input.disabled=true;try{const response=await fetch(location.href,{method:'POST',body:data,credentials:'same-origin'}),result=await response.json();if(!response.ok||!result.success)throw new Error(result.message||'Teslim tarihi kaydedilemedi.');input.dataset.savedValue=input.value;input.closest('tr')?.querySelector('[data-due-date]')&&(input.closest('tr').querySelector('[data-due-date]').textContent=result.due_date?result.due_date.split('-').reverse().join('.'):'—');}catch(error){alert(error.message||'Teslim tarihi kaydedilemedi.');input.value=input.dataset.savedValue||'';}finally{input.disabled=false;}})});document.querySelectorAll('.sgk-list tbody tr[data-edit-url]').forEach(row=>row.addEventListener('dblclick',event=>{if(event.target.closest('input,button,a,label'))return;location.href=row.dataset.editUrl;}));</script>
<script>document.querySelectorAll('.delivery-date').forEach(input=>{const picker=document.createElement('input'),button=document.createElement('button');picker.type='date';picker.className='delivery-date-native';button.type='button';button.className='delivery-date-picker';button.setAttribute('aria-label','Takvimden teslim tarihi seç');button.innerHTML='<i class="icon-base ti tabler-calendar"></i>';input.after(picker);input.after(button);button.addEventListener('click',()=>{picker.value=deliveryIso(input.value);if(typeof picker.showPicker==='function')picker.showPicker();else picker.focus();});picker.addEventListener('change',()=>{input.value=picker.value?picker.value.split('-').reverse().join('.'):'';input.dispatchEvent(new Event('change',{bubbles:true}));});});</script>
