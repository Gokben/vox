...
$sql = 'SELECT patients.*,branches.name AS branch_name FROM patients LEFT JOIN branches ON branches.id=patients.branch_id' . $whereSql . ' ORDER BY import_order,patients.id LIMIT ' . $perPage . ' OFFSET ' . $offset;
...
<script>
(()=>{
 const columns=['No','Tarih','Ad Soyad','T.C. Kimlik No','Telefon 1','Telefon 2','Doğum Tarihi','Adres','Sosyal Güvence','Rapor','Şube / Hizmet','Başvuru Kaynağı','Açıklama','Sonuç','İlgili','Eylemler'];
 ...
 picker.querySelector('[data-hide-optional]').addEventListener('click',()=>{visible=columns.map((_,i)=>[0,2,3,4,5,15].includes(i));apply()});
