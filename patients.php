...
$activeSearchColumns = $hasSearchColumnSelection ? $requestedSearchColumns : $searchColumnKeys;
...
if ($q !== '') {
    ...
    foreach ($activeSearchColumns as $column) {
        ...
        elseif ($column === 'result') $addSearch("(CASE WHEN patients.approval=1 THEN 'Onay' WHEN patients.considering=1 THEN 'Düşünecek' WHEN patients.rejected=1 THEN 'Red' ELSE '' END) LIKE ?");
...
<style>.year-select{height:39px;border:1px solid #d5d3de;border-radius:6px;background:var(--card);color:var(--text);padding:0 12px;font:inherit}.vuexy-dt-card.column-menu-open{overflow:visible}</style>
<script>
(()=>{
