...
function patient_page_url(int $target, string $q, int $length): string { global $year,$showAll,$dateSort; return url('patients.php?' . http_build_query(['year'=>$year,'q'=>$q,'all'=>$showAll?'1':'','sort'=>$dateSort,'length'=>$length,'page'=>$target])); }
function patient_date_sort_url(): string { global $year,$showAll,$dateSort,$q,$perPage; return url('patients.php?' . http_build_query(['year'=>$year,'q'=>$q,'all'=>$showAll?'1':'','sort'=>$dateSort==='date_asc'?'date_desc':'date_asc','length'=>$perPage,'page'=>1])); }
...
(()=>{
 ...
 table.querySelectorAll('tbody tr').forEach(row=>{row.style.cursor='pointer';row.addEventListener('click',event=>{if(event.target.closest('a,button,input,form'))return;const services=row.querySelector('a[href*="patient-followup.php?id="]');if(services)window.location.href=services.href});});
