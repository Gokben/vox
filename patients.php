...
// Reuse list preferences across pagination and reopening this patient list.
foreach (['length','sort','search_columns'] as $preference) {
    if (array_key_exists($preference, $_GET)) $_SESSION['patient_list_preferences'][$preference] = (string)$_GET[$preference];
    elseif (isset($_SESSION['patient_list_preferences'][$preference])) $_GET[$preference] = $_SESSION['patient_list_preferences'][$preference];
}
$showAll = ($_GET['all'] ?? '') === '1';
...
$searchColumnKeys = ['no','date','name','national_id','phone_primary','phone_secondary','birth_date','address','social_security','report','service_location','application_detail','source','service_name','notes','result','contact'];
...
if ($q !== '') {
    ...
    foreach ($activeSearchColumns as $column) {
        ...
        elseif ($extendedSchemaReady && $column === 'result') $addSearch('EXISTS (SELECT 1 FROM patient_services AS search_services WHERE search_services.patient_id=patients.id AND search_services.id=(SELECT MAX(latest_service.id) FROM patient_services AS latest_service WHERE latest_service.patient_id=patients.id) AND search_services.result_name LIKE ?)');
        ...
        elseif ($extendedSchemaReady && $column === 'service_name') $addSearch('EXISTS (SELECT 1 FROM patient_services AS search_services WHERE search_services.patient_id=patients.id AND search_services.service_name LIKE ?)');
...
try {
    ...
    $fallbackSql = 'SELECT patients.*,NULL AS branch_name,patients.service_type AS service_type_name,NULL AS source_name,NULL AS service_contact_person,NULL AS service_card_location,NULL AS latest_service_name,NULL AS latest_service_result FROM patients'
...
(()=>{
 ...
 const searchKeys=['no','date','name','national_id','phone_primary','phone_secondary','birth_date','address','social_security','report','service_location','application_detail','source','service_name','notes','result','contact',null];
 ...
 const widthLimits=[[54,70],[82,102],[135,220],[108,145],[94,130],[94,130],[92,122],[220,380],[105,170],[105,190],[95,155],[135,260],[105,190],[105,190],[155,320],[82,125],[125,220],[158,158]];
...
<script src="<?=e(url('assets/patient-delete-confirm.js?v=1'))?>" defer></script>
<?php patient_footer();
