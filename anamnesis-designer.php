...
<main class="designer-page">
  ...
  <form id="visual-designer-form" method="post"><input type="hidden" name="csrf" value="<?=csrf()?>"><input id="grapesjs-project" type="hidden" name="grapesjs_project"><div id="gjs"></div><footer><button type="button" id="paged-preview">A4 Ön Görüntü</button><button type="submit">Tasarımı Kaydet</button></footer></form>
</main>
<link rel="stylesheet" href="<?=url('assets/vendor/grapesjs/grapes.min.css')?>">
<style>
.designer-page{max-width:1440px;margin:0 auto;padding:28px 20px}.designer-head{display:flex;justify-content:space-between;gap:18px;align-items:start;margin-bottom:18px}.designer-head h1{margin:0 0 6px;font-size:24px}.designer-head p{margin:0;color:#6d6875}.designer-back,#visual-designer-form footer button{display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:7px;padding:11px 16px;background:#19a94b;color:#fff;text-decoration:none;font-weight:700;cursor:pointer}#visual-designer-form footer{display:flex;justify-content:flex-end;gap:10px;padding:14px 0}#paged-preview{background:#30435d!important}#gjs{height:calc(100vh - 220px);min-height:620px;border:1px solid #d9dde5}.vox-message{padding:12px 16px;border-radius:7px}.vox-message.success{background:#daf5e3;color:#0d7130}.vox-message.error{background:#ffe3e3;color:#a21d1d}@media(max-width:700px){.designer-page{padding:18px 10px}.designer-head{flex-direction:column}#gjs{height:70vh;min-height:500px}}
</style>
...
(() => {
  ...
  const editor = grapesjs.init({container:'#gjs',height:'100%',storageManager:false,fromElement:false,blockManager:{appendTo:'.gjs-blocks-c',blocks:[
