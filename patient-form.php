...
try {
    ...
    if (!in_array('phone_primary_country_code', $ratingColumns, true)) {
        $ratingPdo->exec($ratingPdo->getAttribute(PDO::ATTR_DRIVER_NAME) === 'sqlite'
            ? "ALTER TABLE patients ADD COLUMN phone_primary_country_code TEXT NOT NULL DEFAULT '+90'"
            : "ALTER TABLE patients ADD COLUMN phone_primary_country_code VARCHAR(8) NOT NULL DEFAULT '+90'");
    }
    if (!in_array('phone_secondary_country_code', $ratingColumns, true)) {
        $ratingPdo->exec($ratingPdo->getAttribute(PDO::ATTR_DRIVER_NAME) === 'sqlite'
            ? "ALTER TABLE patients ADD COLUMN phone_secondary_country_code TEXT NOT NULL DEFAULT '+90'"
            : "ALTER TABLE patients ADD COLUMN phone_secondary_country_code VARCHAR(8) NOT NULL DEFAULT '+90'");
    }
...
$fields = ['branch_id','record_date','full_name','national_id','phone_primary','phone_primary_country_code','proximity_relation','phone_secondary','phone_secondary_country_code','proximity_relation_secondary','birth_date','address','patient_rating','patient_rating_comment','patient_status','social_security','report_status','service_type','service_type_id','source_id','source_detail','anamnesis','notes'];
$patient = array_fill_keys($fields, '');
$patient['patient_status'] = 'active';
$patient['phone_primary_country_code'] = '+90';
$patient['phone_secondary_country_code'] = '+90';
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    foreach (['phone_primary_country_code', 'phone_secondary_country_code'] as $countryCodeField) {
        if (!in_array($patient[$countryCodeField], ['+90', '+49', '+44', '+1', '+31'], true)) $patient[$countryCodeField] = '+90';
    }
...
<div class="icon-form-row"><label class="icon-form-label">Telefon 1</label><div class="merged-input phone-input"><span class="merged-icon">⌕</span><select class="phone-country-code" name="phone_primary_country_code"><option value="+90" <?=$patient['phone_primary_country_code']==='+90'?'selected':''?>>+90</option><option value="+49" <?=$patient['phone_primary_country_code']==='+49'?'selected':''?>>+49</option><option value="+44" <?=$patient['phone_primary_country_code']==='+44'?'selected':''?>>+44</option><option value="+1" <?=$patient['phone_primary_country_code']==='+1'?'selected':''?>>+1</option><option value="+31" <?=$patient['phone_primary_country_code']==='+31'?'selected':''?>>+31</option></select><input id="phone_primary" name="phone_primary" inputmode="tel" maxlength="14" placeholder="0546 638 67 75" value="<?=e($patient['phone_primary'])?>"><button id="proximity-toggle" class="proximity-toggle" type="button" title="Yakınlık derecesini aç/kapat" aria-label="Yakınlık derecesini aç/kapat" aria-controls="proximity-row" aria-expanded="false" <?=trim($patient['phone_primary'])===''?'disabled':''?>><i class="icon-base ti tabler-users"></i></button></div></div>
