...
<main class="calendar-page">
  <section class="calendar-shell">
    <aside class="calendar-sidebar">
      <a class="calendar-add" href="<?= url('patient-form.php?date='.date('Y-m-d')) ?>">＋ Randevu Ekle</a>
      <a class="calendar-add calendar-add-daily" href="<?= url('appointment-form.php?type=daily_event&date='.date('Y-m-d')) ?>">＋ Günlük Olay Ekle</a>
...
const appointmentEditUrl = <?=json_encode(url('appointment-form.php?id='))?>;
Object.entries(calendarAppointments).forEach(([date, appointments]) => {
  ...
  appointments.forEach(appointment => {
    ...
    link.className = 'calendar-event ' + (dailyEvent ? 'calendar-daily-event' : 'calendar-appointment');
    link.href = appointmentEditUrl + encodeURIComponent(appointment.id);
    const name = document.createElement('b');
    name.textContent = appointment.name;
    const time = document.createElement('small');
    time.textContent = 'Randevu · ' + appointment.time;
    time.textContent = (dailyEvent ? 'Günlük olay' : 'Randevu') + ' · ' + appointment.time;
...
<script>const calendarAddButtons=[...document.querySelectorAll('.calendar-sidebar>.calendar-add')];if(calendarAddButtons.length){const wrapper=document.createElement('div');wrapper.className='calendar-add-actions';calendarAddButtons[0].before(wrapper);calendarAddButtons.forEach(button=>wrapper.append(button));}document.querySelectorAll('.calendar-add:not(.calendar-add-daily),.calendar-new').forEach(link=>{link.href=link.href.replace('patient-form.php','appointment-form.php');if(link.classList.contains('calendar-new'))link.title='Bu tarihe randevu ekle';});</script>
<?php patient_footer(); ?>
