  ...
  <section class="calendar-shell">
    ...
    <section class="calendar-content">
      ...
      <div class="calendar-grid">
        ...
        <?php for($day=1;$day<=$daysInMonth;$day++):$date=$month.'-'.str_pad((string)$day,2,'0',STR_PAD_LEFT);?><div class="calendar-day <?= $date===date('Y-m-d')?'is-today':'' ?>" id="day-<?=$day?>" data-date="<?=$date?>"><div class="day-head"><span class="day-number"><?=$day?></span></div><div class="day-events"></div></div><?php endfor; ?>
      </div>
    </section>
  </section>
  <section class="appointment-list"><header><h2>Randevu Listesi</h2><span><?=count($appointments)?> randevu</span></header><?php if (($_GET['appointment_saved'] ?? '') === '1'): ?><div class="appointment-saved">Randevu kaydedildi.</div><?php endif ?><div class="appointment-list-wrap"><table><thead><tr><th>Tarih</th><th>Saat</th><th>Ad Soyad</th><th>Telefon</th><th>Şube</th><th>İlgilenen Kişi</th><th>Not</th></tr></thead><tbody><?php foreach ($appointments as $appointment): ?><tr><td><?=e(format_date_tr($appointment['appointment_date']))?></td><td><?=e(substr((string)$appointment['appointment_time'],0,5))?></td><td><?=e($appointment['full_name'])?></td><td><?=e(implode(' / ', array_filter([(string)($appointment['phone'] ?? ''),(string)($appointment['phone_secondary'] ?? '')])) ?: '—')?></td><td><?=e($appointment['branch_name'] ?: '—')?></td><td><?=e($appointment['contact_person'] ?: '—')?></td><td><?=e($appointment['note'] ?: '—')?></td></tr><?php endforeach; if (!$appointments): ?><tr><td colspan="7" class="appointment-empty">Bu ay için randevu bulunmuyor.</td></tr><?php endif ?></tbody></table></div></section>
</main>
...
<style>.day-head{display:flex;align-items:center}.calendar-patient-appointment{background:#f3ecff!important;border-color:#7b55c7!important;color:#51318f!important}</style>
<script>
(()=>{
