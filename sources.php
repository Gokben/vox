...
<main class="patient-container personnel-page">
  ...
  <details class="vuexy-form-card definition-accordion"<?=((int)$edit['id'] || $error !== '') ? ' open' : ''?>>
    <summary class="form-card-title"><span><h1><?= (int)$edit['id'] ? 'Başvuru Kaynağını Düzenle' : 'Yeni Başvuru Kaynağı' ?></h1><p>Hasta kartlarında seçimli olarak kullanılacak kaynakları yönetin.</p></span><i class="definition-chevron" aria-hidden="true"></i></summary>
    ...
    <form method="post" action="<?= url('sources.php' . ((int)$edit['id'] ? '?edit=' . (int)$edit['id'] : '')) ?>" class="personnel-form">
      ...
      <div class="personnel-actions"><button><?= (int)$edit['id'] ? 'Güncelle' : 'Kaydet' ?></button><?php if ((int)$edit['id']): ?> <a class="edit-personnel" href="<?=url('sources.php')?>">İptal</a><?php endif; ?></div>
    </form>
  </details>
...
<style>.patient-container.definition-settings{width:100%;max-width:none!important;min-height:100vh;margin:0!important;padding:96px 32px 48px!important}.settings-tabs{display:flex;gap:8px;margin-bottom:20px;flex-wrap:wrap}.vuexy-form-card{background:#fff;border:1px solid #e1e2e8;border-radius:10px;margin-bottom:24px;box-shadow:0 3px 12px #1e283c0f;overflow:hidden}.form-card-title{display:block!important;padding:22px 24px!important;border-bottom:1px solid #e1e2e8!important}.form-card-title h1,.form-card-title h2{margin:0 0 5px!important;color:#2f2b3d!important;font-size:21px!important;line-height:1.25!important}.form-card-title p{margin:0!important;color:#7b7b8d!important}.definition-form{display:flex;gap:16px;align-items:end;padding:24px;flex-wrap:wrap}.definition-form label{display:flex;flex-direction:column;gap:7px}.definition-form input:not([type=checkbox]){height:43px;min-width:240px;border:1px solid #d2d2dc;border-radius:7px;padding:0 12px;background:transparent;color:inherit}.definition-form .check{flex-direction:row;align-items:center;height:43px}.definition-form button,.edit-definition,.delete-definition{border:0;border-radius:7px;padding:11px 18px;text-decoration:none;font-weight:700;background:#19a94b;color:#fff}.delete-definition{background:#df4a4a;margin-left:8px}.delete-definition.disabled{background:#a9adb8;opacity:.65;cursor:not-allowed}.inline{display:inline}.definition-table{width:100%;border-collapse:collapse;min-width:680px}.definition-table th,.definition-table td{padding:14px 18px;border-bottom:1px solid #e1e2e8;text-align:left;white-space:nowrap}.table-responsive{overflow:auto}.vox-message{padding:13px 16px;margin:16px 24px;border-radius:7px}.vox-message.success{background:#daf5e3;color:#0d7130}.vox-message.error{background:#ffe3e3;color:#a21d1d}[data-theme=dark] .vuexy-form-card{background:#2f3349;color:#fff;border-color:#454a63}[data-theme=dark] .form-card-title{border-color:#454a63!important}[data-theme=dark] .form-card-title h1,[data-theme=dark] .form-card-title h2{color:#fff!important}[data-theme=dark] .definition-form input:not([type=checkbox]){border-color:#5a607b;color:#fff}[data-theme=dark] .form-card-title p{color:#c4c7d6!important}@media(max-width:900px){.patient-container.definition-settings{padding:92px 14px 30px!important}}@media(max-width:720px){.definition-form label{width:100%}.definition-form input:not([type=checkbox]){min-width:0;width:100%}}</style>
<style>
.definition-accordion>summary{position:relative;display:flex!important;align-items:center;padding-right:64px!important;cursor:pointer;list-style:none;user-select:none}
.definition-accordion>summary::-webkit-details-marker{display:none}.definition-accordion>summary::marker{display:none;content:""}
.definition-chevron{position:absolute;right:24px;top:50%;display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;font-style:normal;font-size:20px;font-weight:500;line-height:1;transform:translateY(-50%);transition:transform .2s ease}
.definition-chevron::before{content:">"}.definition-accordion[open] .definition-chevron{transform:translateY(-50%) rotate(90deg)}
.definition-accordion:not([open])>summary{border-bottom:0!important}
</style>
<?php patient_footer(); ?>
