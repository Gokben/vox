...
$deliveryConsumables = sale_consumable_lines($sale);
$deliveryConsumableNames = [];
if ($deliveryConsumables) {
    $stockIds = array_values(array_unique(array_column($deliveryConsumables, 'stock_id')));
    $stockQuery = $pdo->prepare('SELECT id,stock_name FROM stock_cards WHERE id IN (' . implode(',', array_fill(0, count($stockIds), '?')) . ')');
    $stockQuery->execute($stockIds);
    $deliveryConsumableNames = $stockQuery->fetchAll(PDO::FETCH_KEY_PAIR);
}
$moneyNumber = static function ($value): float {
...
if ($discountIsSummary) {
    $discountDescription = '%' . number_format($moneyNumber($discountParts[1]), 2, ',', '.');
    $discountTotal = $moneyNumber($discountParts[2]);
    $discountAmountText = number_format($discountTotal, 2, ',', '.') . ' ₺';
} else {
    $discountTotal = $moneyNumber($discountRaw);
    if (str_contains($discountRaw, '%')) {
        $discountDescription = $discountRaw;
    } elseif ($discountRaw !== '') {
        $discountAmountText = number_format($discountTotal, 2, ',', '.') . ' ₺';
    }
}
