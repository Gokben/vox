...
$resultsReturn = 'patient-results.php?' . http_build_query(['result'=>$result,'q'=>$q,'page'=>$page]);
?>
<style>
      ...
      <table class="results-table">
        ...
        <?php foreach ($rows as $patient): $label = patient_result_label($patient); $class = $label==='Onay'?'approved':($label==='Düşünecek'?'considering':($label==='Red'?'rejected':'none')); ?>
          <tr>
            ...
            <td><a class="result-edit" href="<?=url('patient-form.php?'.http_build_query(['id'=>(int)$patient['id'],'return'=>$resultsReturn]))?>">Düzenle</a></td>
          </tr>
        <?php endforeach; ?>
