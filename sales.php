...
<main class="sales-page"><section class="sales-card"><header class="sales-head"><h1>Satışlar</h1><p>Stokta bulunan tüm ürünler.</p></header><div class="sales-search"><i class="icon-base ti tabler-search"></i><select id="sales-stock-type" aria-label="Stok tipi"><option value="">Tüm stok tipleri</option><?php foreach ($stockTypes as $stockType): ?><option value="<?=e($stockType)?>" <?=$stockType===$selectedStockType?'selected':''?>><?=e($stockType)?></option><?php endforeach; ?></select><input id="sales-search" type="search" placeholder="Stok kodu, stok adı, marka veya model ara" autocomplete="off"></div><div class="sales-scroll"><table class="sales-table"><thead><tr><th>Stok Tipi</th><th>Stok Kodu</th><th>Stok Adı</th><th>Marka</th><th>Model</th><th>Görsel</th><th>Stok Miktarı</th></tr></thead><tbody><?php foreach ($stocks as $stock): ?><tr><td><?=e($stock['stock_type'] ?: '—')?></td><td><?=e($stock['stock_code'])?></td><td><?=e($stock['stock_name'])?></td><td><?=e($stock['brand'] ?: '—')?></td><td><?=e($stock['model'] ?: '—')?></td><td><?php if (!empty($stock['image_path'])): ?><img class="sales-image" src="<?=e(url($stock['image_path']))?>" alt="<?=e($stock['stock_name'])?>"><?php else: ?>—<?php endif ?></td><td class="sales-stock"><?=e((string)$stock['stock_quantity'])?></td></tr><?php endforeach; ?><?php if (!$stocks): ?><tr><td class="sales-empty" colspan="7">Stokta ürün bulunmuyor.</td></tr><?php endif; ?></tbody></table></div></section></main>
