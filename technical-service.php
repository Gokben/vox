...
function repair_value(string $details,string $key): string {$data=json_decode($details,true);$value=is_array($data)?($data[$key]??''):'';return is_array($value)?implode(', ',$value):(string)$value;}
