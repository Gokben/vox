...
function social_security_definitions(): array {
    ...
    $pdo->exec($driver === 'sqlite'
        ? 'CREATE TABLE IF NOT EXISTS social_security_bootstrap_meta (meta_key TEXT PRIMARY KEY, created_at TEXT DEFAULT CURRENT_TIMESTAMP)'
        : 'CREATE TABLE IF NOT EXISTS social_security_bootstrap_meta (meta_key VARCHAR(100) PRIMARY KEY, created_at DATETIME DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
    $initialized = (bool)$pdo->query("SELECT 1 FROM social_security_bootstrap_meta WHERE meta_key='initial_import_done' LIMIT 1")->fetchColumn();
    $definitionCount = (int)$pdo->query('SELECT COUNT(*) FROM social_security_definitions')->fetchColumn();

    // Varsayılanlar ve eski hasta kayıtlarından aktarılan değerler yalnızca ilk kurulumda taranır.
    if (!$initialized || $definitionCount === 0) {
        ...
        $done = $pdo->prepare($driver === 'sqlite'
            ? "INSERT OR REPLACE INTO social_security_bootstrap_meta(meta_key) VALUES ('initial_import_done')"
            : "INSERT INTO social_security_bootstrap_meta(meta_key) VALUES ('initial_import_done') ON DUPLICATE KEY UPDATE meta_key=VALUES(meta_key)");
        $done->execute();
    }
