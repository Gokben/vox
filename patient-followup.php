...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cash_update_only' && isset($_POST['payment_records_json'])) {
        ...
        try {
            ...
            $payments = cash_payment_decode_records((string)$_POST['payment_records_json'], cash_payment_money($details['sales_payment_amount'] ?? 0));
...
const initializeSalesScreen=()=>{
  ...
  window.addEventListener('click',event=>{const button=event.target.closest('form[action*="cash.php"] footer button');if(!button||button.matches('[data-cash-close],[aria-label="Bir gelir kaydı daha ekle"]'))return;if(!validIncomeDates()){event.preventDefault();event.stopImmediatePropagation();}},true);
...
(()=>{
  const setup=()=>{
    ...
    const normalizeFooter=()=>{
      ...
      const save=[...footer.querySelectorAll('button')].find(button=>!button.matches('.repair-cancel,[data-cash-close],[aria-label="Bir gelir kaydı daha ekle"]'));
