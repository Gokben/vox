...
<label class="service-field">Hizmet Adı<select name="service_name"><option value="">Seçiniz</option><?php foreach($serviceNames as $serviceName):?><option value="<?=e($serviceName['name'])?>"><?=e($serviceName['name'])?></option><?php endforeach?></select></label><label id="sales-stock-control" class="service-field" hidden>Satış Stoğu<button type="button" id="sales-stock-open">Stok Seç</button><small id="sales-stock-selected">Stok seçilmedi</small></label><label class="service-field">Sonuç<select name="result_name"><option>Beklemede</option><option>Onay</option><option>Düşünecek</option><option>Ret</option><option>Tamamlandı</option><option>İptal</option></select></label>
...
<script>
(() => {
  const form=document.getElementById('service-card-form'), service=form?.querySelector('[name="service_name"]'), modal=document.getElementById('sales-stock-modal'), control=document.getElementById('sales-stock-control'), value=document.getElementById('sales_stock_id'), selected=document.getElementById('sales-stock-selected'), search=document.getElementById('sales-stock-search');
  if(!form||!service||!modal||!control||!value||!selected)return;
  const items=[...modal.querySelectorAll('.sales-stock-item')], isSales=()=>service.value.trim().toLocaleLowerCase('tr-TR')==='satış', open=()=>{modal.hidden=false;modal.setAttribute('aria-hidden','false');search?.focus()}, close=()=>{modal.hidden=true;modal.setAttribute('aria-hidden','true')}, refresh=()=>{const item=items.find(x=>x.dataset.id===value.value);selected.textContent=item?.dataset.label||'Stok seçilmedi'};
  service.addEventListener('change',()=>{control.hidden=!isSales();if(isSales())open();else{value.value='';refresh();close()}});
  document.getElementById('sales-stock-open')?.addEventListener('click',open);modal.querySelectorAll('[data-sales-close]').forEach(x=>x.addEventListener('click',close));items.forEach(item=>item.addEventListener('click',()=>{value.value=item.dataset.id||'';refresh();close()}));search?.addEventListener('input',()=>{const q=search.value.trim().toLocaleLowerCase('tr-TR');items.forEach(item=>item.hidden=!!q&&!(item.dataset.search||'').includes(q))});control.hidden=!isSales();refresh();
})();
</script>
<script>
(() => {
