...
const initializeSalesScreen=()=>{
  ...
  const addExtraDevice=number=>{
    ...
    const sync=()=>{if(!brand.value){model.replaceChildren(new Option('Önce marka seçiniz',''));model.disabled=true;return;}const invoice=(salesInvoiceInput?.value||'').trim(),historicalModels=invoice?salesExitSerials.filter(item=>item.stock_type==='İşitme Cihazı'&&item.brand===brand.value&&String(item.invoice_no||'').trim()===invoice).map(item=>item.model):[];fillSelect(model,[...hearingDeviceStocks.filter(stock=>stock.brand===brand.value).map(stock=>stock.model),...historicalModels],'Model seçiniz');model.disabled=false;};
