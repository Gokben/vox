...
$saleProductDeleteLocked = $saleStockLocked && $serviceNameLocked;
...
$showSalesDetailsButton = $editId > 0 && $saleStockLocked;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedServiceName === 'Satış') {
            ...
            $addDeviceExit = static function (string $type, string $brand, string $model, string $serial, int $quantity = 1) use ($findStock, $addExit, $movementDate, $description, $accountId, $invoiceNo): void {
...
const initializeSalesScreen=()=>{
  ...
  const salesDateInput=detailsModal?.querySelector('[name="sales_sale_date"]'),deviceSerialInput=detailsModal?.querySelector('[name="sales_device_serial"]'),deviceDiscountInput=detailsModal?.querySelector('[name="sales_device_discount_rate"]'),deviceNetPriceInput=detailsModal?.querySelector('[name="sales_device_net_price"]');
  ...
  const fillSerialOptions=(select,stocks)=>{if(!(select instanceof HTMLSelectElement))return;const current=select.dataset.value||select.value||'',serials=[...new Set(stocks.flatMap(stock=>{try{const values=JSON.parse(stock.serial_numbers||'[]');return Array.isArray(values)?values.map(value=>String(value).trim()).filter(Boolean):[];}catch(_){return [];}}))];select.replaceChildren(new Option(serials.length?'Seri no seçiniz':'Seri numarası bulunamadı',''));serials.forEach(serial=>select.add(new Option(serial,serial)));select.value=serials.includes(current)?current:'';delete select.dataset.value;select.disabled=!serials.length;};
  ...
  const refreshProductDeleteButtons=()=>{const showButtons=!saleStockLocked||activeProductLineCount()>1;detailsModal?.querySelectorAll('.sales-product-cancel').forEach(button=>button.hidden=!showButtons);};
  const addProductCancel=(container,names,onClear)=>{const button=document.createElement('button');button.type='button';button.className='repair-cancel sales-product-cancel';button.textContent='×';button.title='Ürünü kaldır';button.setAttribute('aria-label','Ürünü kaldır');button.addEventListener('click',()=>{const hasSavedProduct=names.some(name=>String(savedSaleProducts[name]??'').trim()!=='');if(saleStockLocked&&hasSavedProduct&&activeProductLineCount()<=1){alert('Hasta ödeme yapmış. Son ürün kalemini silemezsiniz.');return;}productWasRemoved=true;clearFields(names);onClear();updateTotalAmount();refreshProductDeleteButtons();});container.append(button);refreshProductDeleteButtons();return button;};
