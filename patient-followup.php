...
require_once __DIR__ . '/cash-payment-records.php';
require_once __DIR__ . '/sale-consumables.php';
...
$chargerEditStatement = $pdo->prepare('SELECT id,record_no FROM patient_services WHERE id=? AND patient_id=?');
$chargerEditStatement->execute([(int)($_GET['edit'] ?? 0), $id]);
$chargerEditService = $chargerEditStatement->fetch() ?: [];
$chargerEditId = (int)($chargerEditService['id'] ?? 0);
$chargerEditDescription = $chargerEditId > 0 ? 'Hizmet kartı satışı: ' . (string)$chargerEditService['record_no'] : '';
$chargerDeviceStatement = $pdo->prepare("SELECT s.id,s.brand,s.model,s.sale_price,(SELECT m.serial_numbers FROM stock_movements m WHERE m.stock_id=s.id AND m.movement_type='Giriş' AND COALESCE(m.serial_numbers,'') NOT IN ('','[]') ORDER BY m.movement_date DESC,m.id DESC LIMIT 1) AS serial_numbers FROM stock_cards s INNER JOIN (SELECT stock_id,SUM(CASE WHEN movement_type='Giriş' THEN quantity WHEN movement_type='Çıkış' THEN -quantity ELSE 0 END) AS stock_quantity FROM stock_movements GROUP BY stock_id) q ON q.stock_id=s.id AND (q.stock_quantity>=1 OR (? > 0 AND EXISTS (SELECT 1 FROM stock_movements own_exit WHERE own_exit.stock_id=s.id AND own_exit.movement_type='Çıkış' AND (own_exit.service_id=? OR own_exit.description=?)))) WHERE s.stock_type=? ORDER BY s.brand,s.model,s.id");
$chargerDeviceStatement->execute([$chargerEditId, $chargerEditId, $chargerEditDescription, 'Şarj Cihazı']);
...
$serialMovementStatement = $pdo->prepare("SELECT m.stock_id,s.stock_type,m.movement_type,m.serial_numbers,m.service_id,m.description FROM stock_movements m INNER JOIN stock_cards s ON s.id=m.stock_id WHERE s.stock_type IN (?,?) AND COALESCE(m.serial_numbers,'') NOT IN ('','[]') ORDER BY m.id");
$serialMovementStatement->execute(['İşitme Cihazı','Şarj Cihazı']);
$availableSerials = [];
foreach ($serialMovementStatement as $serialMovement) {
    if ($chargerEditId > 0 && $serialMovement['stock_type'] === 'Şarj Cihazı' && $serialMovement['movement_type'] === 'Çıkış'
        && ((int)($serialMovement['service_id'] ?? 0) === $chargerEditId || (string)($serialMovement['description'] ?? '') === $chargerEditDescription)) continue;
...
if (trim((string)($serviceCard['service_name'] ?? '')) === 'Satış') {
    try {
        $cashPaymentStatement = $pdo->prepare("SELECT id,transaction_date,amount,description,payment_type,installment_count,bank_name,commission_rate,current_account_id,term_schedule FROM cash_transactions WHERE source_url=? AND transaction_type='income' ORDER BY id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cash_cancel_last_income') {
        header('Content-Type: application/json; charset=utf-8');
        try {
            if (!$linkedSaleState['sale']) throw new RuntimeException('Satış kartı bulunamadı.');
            $remaining = cash_payment_cancel_last($pdo, url('patient-followup.php?id=' . $id), $cashDeleteId);
            echo json_encode(['success'=>true, 'records'=>$remaining], JSON_UNESCAPED_UNICODE);
        } catch (Throwable $error) {
            http_response_code(422);
            echo json_encode(['success'=>false, 'message'=>$error instanceof PDOException ? 'Gelir kaydı iptal edilemedi.' : $error->getMessage()], JSON_UNESCAPED_UNICODE);
        }
        exit;
    }
    ...
    if ($action === 'cash_update_only' && isset($_POST['payment_records_json'])) {
        header('Content-Type: application/json; charset=utf-8');
        try {
            $saleQuery = $pdo->prepare("SELECT sales_details FROM patient_services WHERE id=? AND patient_id=? AND service_name='Satış'");
            $saleQuery->execute([$postedEditId, $id]);
            $saleDetails = $saleQuery->fetchColumn();
            if ($saleDetails === false) throw new RuntimeException('Satış kartı bulunamadı.');
            $details = json_decode((string)$saleDetails, true) ?: [];
            $payments = cash_payment_decode_records((string)$_POST['payment_records_json'], cash_payment_sale_total($details));
            $source = url('patient-followup.php?id=' . $id);
            cash_payment_save_batch($pdo, $payments, $source, (int)($_SESSION['user']['id'] ?? 0));
            $query = $pdo->prepare("SELECT id,transaction_date,amount,description,payment_type,installment_count,bank_name,commission_rate,current_account_id,term_schedule FROM cash_transactions WHERE source_url=? AND transaction_type='income' ORDER BY id");
            $query->execute([$source]);
            echo json_encode(['success'=>true,'records'=>$query->fetchAll()], JSON_UNESCAPED_UNICODE);
        } catch (Throwable $error) {
            http_response_code(422);
            echo json_encode(['success'=>false,'message'=>$error instanceof PDOException ? 'Ödemeler kaydedilemedi.' : $error->getMessage()], JSON_UNESCAPED_UNICODE);
        }
        exit;
    }
    if ($action === 'cash_update_only' && $postedEditId > 0) {
        ...
        $moneyValue = static fn(mixed $value): float => cash_payment_money($value);
        ...
        $saleTotal = cash_payment_sale_total($saleTotalDetails);
        ...
        if (abs(($primaryTotal + $extraTotal) - $saleTotal) > 0.009) {
            ...
            $_SESSION['income_validation_draft'] = [
                ...
                'transaction_date' => (string)($_POST['cash_update_extra_date'] ?? ''),
    ...
    if ($action === 'cash_update_only' || (int)($_POST['cash_update_id'] ?? 0) > 0) {
        try {
            $payments = [];
            foreach (['' => 'cash_update_', 'extra_' => 'cash_update_extra_'] as $slot => $prefix) {
                if ($slot && trim((string)($_POST[$prefix . 'payment_type'] ?? '')) === '') continue;
                $data = [];
                foreach (['description','amount','payment_type','installment_count','bank_name','commission_rate','current_account_id'] as $field) $data[$field] = $_POST[$prefix . $field] ?? '';
                $data['transaction_date'] = $_POST[$prefix . 'date'] ?? '';
                $data['term_schedule'] = $_POST[$prefix . 'term_schedule'] ?? '';
                $payments[] = ['id' => (int)($_POST[$prefix . 'id'] ?? 0), 'record' => cash_payment_record($data)];
            }
            cash_payment_save_batch($pdo, $payments, url('patient-followup.php?id=' . $id), (int)($_SESSION['user']['id'] ?? 0));
        } catch (Throwable $error) {
            $message = $error instanceof PDOException ? 'Ödeme kayıtları kaydedilemedi.' : $error->getMessage();
            ...
            $_SESSION['income_validation_error'] = $message;
            redirect('patient-followup.php?id=' . $id . '&edit=' . $postedEditId . '&open_income_record=1' . $serviceNavigationContext);
        }
    }
    if ($action === 'cash_update_only') {
        if ((string)($_POST['ajax'] ?? '') === '1') {
            $cashRefreshStatement = $pdo->prepare("SELECT id,transaction_date,amount,description,payment_type,installment_count,bank_name,commission_rate,current_account_id,term_schedule FROM cash_transactions WHERE source_url=? AND transaction_type='income' ORDER BY id");
    ...
    if (($postedSalesDetails['sales_charger_promotion'] ?? 'Hayır') === 'Evet') {
        foreach (['sales_charger_net_price', 'sales_charger_sgk', 'sales_charger_discount_rate'] as $chargerMoneyKey) {
            $postedSalesDetails[$chargerMoneyKey] = '0,00';
        }
    }
    ...
    if ($postedServiceName === 'Satış') {
        $hearingSides = json_decode((string)$values['sales_details'], true);
        if (is_array($hearingSides)) {
            $firstSide = ($hearingSides['sales_device_side'] ?? '') === 'sol' ? 'sol' : 'sağ';
            $hearingSides['sales_device_side'] = $firstSide;
            if (($hearingSides['sales_hearing_pair_mode'] ?? '') === 'same') {
                $hearingSides['sales_device_2_brand'] = (string)($hearingSides['sales_brand'] ?? '');
                $hearingSides['sales_device_2_model'] = (string)($hearingSides['sales_model'] ?? '');
            }
            if (trim((string)($hearingSides['sales_device_2_brand'] ?? '')) !== ''
                || trim((string)($hearingSides['sales_device_2_model'] ?? '')) !== ''
                || trim((string)($hearingSides['sales_device_2_serial'] ?? '')) !== '') {
                $hearingSides['sales_device_2_side'] = $firstSide === 'sağ' ? 'sol' : 'sağ';
            }
            $values['sales_details'] = json_encode($hearingSides, JSON_UNESCAPED_UNICODE);
        }
    }
    ...
    if ($postedServiceName === 'Satış') {
        try {
            $validatedSalesDetails = json_decode((string)$values['sales_details'], true) ?: [];
            if (($validatedSalesDetails['sales_hearing_pair_mode'] ?? '') === 'same'
                && trim((string)($validatedSalesDetails['sales_device_serial'] ?? '')) !== ''
                && (string)$validatedSalesDetails['sales_device_serial'] === (string)($validatedSalesDetails['sales_device_2_serial'] ?? '')) {
                throw new RuntimeException('İki işitme cihazına farklı seri numaraları seçmelisiniz.');
            }
            $chargerBrand = trim((string)($validatedSalesDetails['sales_charger_brand'] ?? ''));
            $chargerModel = trim((string)($validatedSalesDetails['sales_charger_model'] ?? ''));
            $chargerSerial = trim((string)($validatedSalesDetails['sales_charger_serial'] ?? ''));
            if ($chargerBrand !== '' || $chargerModel !== '' || $chargerSerial !== '') {
                if ($chargerBrand === '' || $chargerModel === '' || $chargerSerial === '') {
                    throw new RuntimeException('Şarj cihazı satışına devam etmek için marka, model ve seri numarası seçmelisiniz.');
                }
                $chargerStockStatement = $pdo->prepare("SELECT id FROM stock_cards WHERE stock_type='Şarj Cihazı' AND brand=? AND model=? ORDER BY id");
                $chargerStockStatement->execute([$chargerBrand, $chargerModel]);
                $chargerStockIds = array_map('intval', $chargerStockStatement->fetchAll(PDO::FETCH_COLUMN));
                if (!$chargerStockIds) throw new RuntimeException('Seçilen şarj cihazının stok kartı bulunamadı.');
                $chargerSerialStatement = $pdo->prepare("SELECT movement_type,serial_numbers,service_id,description FROM stock_movements WHERE stock_id=? AND COALESCE(serial_numbers,'') NOT IN ('','[]') ORDER BY id");
                $currentRecordNo = trim((string)($serviceCard['record_no'] ?? ''));
                $chargerStockId = 0;
                $serialEntered = false;
                foreach ($chargerStockIds as $candidateStockId) {
                    $chargerSerialStatement->execute([$candidateStockId]);
                    $candidateEntered = false;
                    $candidateSold = false;
                    foreach ($chargerSerialStatement as $serialMovement) {
                        $movementSerials = json_decode((string)$serialMovement['serial_numbers'], true);
                        if (!is_array($movementSerials) || !in_array($chargerSerial, array_map('strval', $movementSerials), true)) continue;
                        if ((string)$serialMovement['movement_type'] === 'Giriş') {
                            $candidateEntered = true;
                            continue;
                        }
                        $belongsToCurrentSale = $postedEditId > 0 && (
                            (int)($serialMovement['service_id'] ?? 0) === $postedEditId
                            || ($currentRecordNo !== '' && str_starts_with((string)($serialMovement['description'] ?? ''), 'Hizmet kartı satışı: ' . $currentRecordNo))
                        );
                        if (!$belongsToCurrentSale) $candidateSold = true;
                    }
                    if ($candidateEntered) $serialEntered = true;
                    if ($candidateEntered && !$candidateSold) { $chargerStockId = $candidateStockId; break; }
                }
                if (!$serialEntered) throw new RuntimeException('Seçilen seri numarası şarj cihazları stok listesinde bulunamadı.');
                if (!$chargerStockId) throw new RuntimeException('Seçilen şarj cihazı seri numarası daha önce satılmış. Başka bir seri numarası seçin.');
                $validatedSalesDetails['sales_charger_stock_id'] = $chargerStockId;
                $values['sales_details'] = json_encode($validatedSalesDetails, JSON_UNESCAPED_UNICODE);
            }
            $consumableLines = sale_consumable_lines($validatedSalesDetails);
            $checkConsumable = $pdo->prepare("SELECT id FROM stock_cards WHERE id=? AND stock_type IN ('Sarf Malzeme','Pil')");
            foreach ($consumableLines as $line) {
                $checkConsumable->execute([$line['stock_id']]);
                if (!$checkConsumable->fetchColumn()) throw new RuntimeException('Sarf malzeme stok kartı bulunamadı.');
            }
        } catch (RuntimeException $error) {
            if (isset($_POST['save_sales_details'])) {
                http_response_code(422);
                header('Content-Type: text/plain; charset=utf-8');
                exit($error->getMessage());
            }
            $_SESSION['service_integrity_error'] = $error->getMessage();
            redirect('patient-followup.php?id=' . $id . '&edit=' . $postedEditId . '&open_sales_details=1' . $serviceNavigationContext);
        }
    }
    ...
    if ($postedServiceName === 'Satış' && !$saleCancelled) {
            ...
            $findStockById = $pdo->prepare('SELECT id FROM stock_cards WHERE id=? AND stock_type=? AND brand=? AND model=? LIMIT 1');
            ...
            $addDeviceExit = static function (string $type, string $brand, string $model, string $serial, int $quantity = 1, int $preferredStockId = 0) use ($findStock, $findStockById, $addExit, $existingInvoiceSerialExit, $movementDate, $description, $accountId, $invoiceNo, $savedServiceId): void {
                ...
                if ($preferredStockId > 0) {
                    $findStockById->execute([$preferredStockId, $type, $brand, $model]);
                    $stockId = (int)$findStockById->fetchColumn();
                } else {
            ...
            $addDeviceExit('Şarj Cihazı', trim((string)($salesDetails['sales_charger_brand'] ?? '')), trim((string)($salesDetails['sales_charger_model'] ?? '')), trim((string)($salesDetails['sales_charger_serial'] ?? '')), 1, (int)($salesDetails['sales_charger_stock_id'] ?? 0));
            foreach (sale_consumable_lines($salesDetails) as $line) {
                $consumableDescription = $description . ($line['promotion'] ? ' — Promosyonlu sarf malzeme' : '');
                $addExit->execute([$line['stock_id'], 'Çıkış', $line['quantity'], $movementDate, $consumableDescription, $accountId, $invoiceNo ?: null, null, $savedServiceId]);
            }
    }
    if ((string)($_POST['ajax'] ?? '') === 'repair_fee_prepare') {
        header('Content-Type: application/json; charset=utf-8');
...
// Keep already sold consumables selectable when their remaining stock is zero.
$savedConsumableDetails = json_decode((string)($serviceCard['sales_details'] ?? ''), true) ?: [];
try { $savedConsumableIds = array_column(sale_consumable_lines($savedConsumableDetails), 'stock_id'); } catch (RuntimeException $error) { $savedConsumableIds = []; }
$missingConsumableIds = array_values(array_diff(array_unique($savedConsumableIds), array_column($consumableStocks, 'id')));
if ($missingConsumableIds) {
    $query = $pdo->prepare("SELECT s.id,s.stock_code,s.stock_name,s.stock_type,s.sale_price,COALESCE((SELECT NULLIF(m.unit,'') FROM stock_movements m WHERE m.stock_id=s.id AND m.movement_type='Giriş' ORDER BY m.movement_date DESC,m.id DESC LIMIT 1),'Adet') AS unit FROM stock_cards s WHERE s.stock_type IN ('Sarf Malzeme','Pil') AND s.id IN (" . implode(',', array_fill(0,count($missingConsumableIds),'?')) . ")");
    $query->execute($missingConsumableIds);$consumableStocks=array_merge($consumableStocks,$query->fetchAll());
}
patient_header($salesWindow ? 'Satış Kartı - ' . (string)$patient['full_name'] : 'Hizmetler', 'patients');
if ($showForm) echo '<script src="' . e(url('assets/sale-device-payment-guard.js')) . '"></script>';
if ($showForm) echo '<script src="' . e(url('assets/sale-payment-stages.js?v=' . filemtime(__DIR__ . '/assets/sale-payment-stages.js'))) . '" data-owner-account-id="' . (int)($pdo->query("SELECT id FROM current_accounts WHERE code='CR-00' LIMIT 1")->fetchColumn() ?: 0) . '" data-endpoint="' . e(url('patient-followup.php?id=' . $id)) . '" data-edit-id="' . $editId . '"></script>';
if (!$showForm): ?>
<style>
...
.classic-service-form .service-field:has(textarea){grid-column:1/-1!important}
...
document.addEventListener('click',async event=>{
  ...
  const chargerBrand=modal.querySelector('[name="sales_charger_brand"]')?.value||'',chargerModel=modal.querySelector('[name="sales_charger_model"]')?.value||'',chargerSerial=modal.querySelector('[name="sales_charger_serial"]')?.value||'';
  if((chargerBrand||chargerModel||chargerSerial)&&(!chargerBrand||!chargerModel||!chargerSerial)){event.preventDefault();event.stopImmediatePropagation();alert('Şarj cihazı satışına devam etmek için marka, model ve stokta bulunan bir seri numarası seçmelisiniz.');modal.querySelector('[name="sales_charger_serial"]')?.focus();return;}
  ...
  try{
    ...
    if(!response.ok)throw new Error((await response.text()).trim()||'Satış bilgileri kaydedilemedi.');
    ...
    // Satış kartı ayrı pencerede açıldıysa kayıt sonrası adres de o pencereye ait kalmalı.
    // Aksi halde yenileme, aynı hizmetin genel düzenleme ekranını gösterir.
    if(new URL(location.href).searchParams.has('sales_window')){
      responseUrl.searchParams.set('open_sales_details','1');
      responseUrl.searchParams.set('sales_window','1');
    }
  ...
  }catch(error){alert(error.message||'Satış bilgileri kaydedilemedi.');}
...
const initializeSalesScreen=()=>{
  ...
  const sameModelOnlyCard=<?=json_encode($id===498 && $editId===583 && trim((string)($serviceCard['record_no'] ?? ''))==='VK-2024')?>;
  ...
  // Include only the current sale's saved products when stock has been exhausted.
  const savedProductCatalog=<?=json_encode($stockCards, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>;
  const includeSavedProduct=(stocks,brand,model,type)=>{
    if(!brand||!model||stocks.some(stock=>stock.brand===brand&&stock.model===model))return;
    const card=savedProductCatalog.find(stock=>stock.brand===brand&&stock.model===model&&stock.stock_type===type);
    stocks.push({...card,brand,model,serial_numbers:'[]'});
  };
  includeSavedProduct(hearingDeviceStocks,savedSaleProducts.sales_brand,savedSaleProducts.sales_model,'İşitme Cihazı');
  includeSavedProduct(hearingDeviceStocks,savedSaleProducts.sales_device_2_brand,savedSaleProducts.sales_device_2_model,'İşitme Cihazı');
  ...
  const parseTurkishMoney=value=>{let normalized=String(value??'').replace(/[^\d,.-]/g,'');if(normalized.includes(','))normalized=normalized.replaceAll('.','').replace(',','.');else if(/^-?\d{1,3}(?:\.\d{3})+$/.test(normalized))normalized=normalized.replaceAll('.','');const amount=Number(normalized);return Number.isFinite(amount)?amount:null;};
  ...
  const syncTotalDiscountMode=()=>{const enabled=hasSecondHearingDevice(),activeProduct=detailsModal?.dataset.productType||'',showTotalDiscount=enabled&&!['Sarf Malzeme','Şarj Cihazı'].includes(activeProduct),label=totalDiscountInput?.closest('label'),deviceDiscountFields=[...detailsModal?.querySelectorAll('[name="sales_device_discount_rate"],[name="sales_device_2_discount_rate"]')||[]];if(detailsModal)detailsModal.dataset.salesLayout=enabled?'dual-hearing':'single-hearing';totalDiscountModeActive=enabled;if(label){label.hidden=!showTotalDiscount;label.style.cssText=showTotalDiscount?'display:flex!important;flex-direction:column;gap:7px':'display:none!important';}if(totalDiscountInput)totalDiscountInput.readOnly=!showTotalDiscount||detailsModal?.dataset.salesLocked==='1';deviceDiscountFields.forEach(field=>{const fieldLabel=field.closest('label');if(fieldLabel){field.disabled=enabled;fieldLabel.hidden=false;fieldLabel.style.cssText=enabled?'display:none!important':'display:flex!important;visibility:visible;pointer-events:auto;flex-direction:column;gap:7px';}});};
  let refreshSameModelSummary=()=>{};
  let savedTotalDifference=null;
  const updateTotalAmount=(preserveDiscountInput=false)=>{const totalField=detailsModal?.querySelector('[name="sales_payment_amount"]');if(!totalField)return;syncTotalDiscountMode();const chargerPromoted=detailsModal?.querySelector('[name="sales_charger_promotion"]')?.value==='Evet';const netFields=['sales_device_net_price','sales_device_2_net_price',...(chargerPromoted?[]:['sales_charger_net_price'])],sgkFields=['sales_device_sgk','sales_device_2_sgk',...(chargerPromoted?[]:['sales_charger_sgk'])];if(chargerPromoted)detailsModal?.querySelectorAll('[name="sales_charger_net_price"],[name="sales_charger_sgk"],[name="sales_charger_discount_rate"]').forEach(field=>{field.value=formatTurkishMoney(0);});let total=netFields.reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);const totalSgk=sgkFields.reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);if(totalSgkInput)totalSgkInput.value=totalSgk>0?formatTurkishMoney(totalSgk):'';const consumablePrice=parseTurkishMoney(detailsModal?.querySelector('[name="sales_consumable_price"]')?.value)||0,consumableQuantity=Number(detailsModal?.querySelector('[name="sales_consumable_quantity"]')?.value)||0;const consumableTotal=detailsModal?.querySelector('[data-consumable-inline-total]')?.value;total+=detailsModal?.querySelector('[name="sales_consumable_promotion"]')?.value==='Evet'?0:detailsModal?.querySelector('[name="sales_consumable_stock_id"]')?.value?(consumableTotal?(parseTurkishMoney(consumableTotal)||0):consumablePrice*consumableQuantity):0;if(totalDiscountInput){const prefixes=['sales_device',...(hasSecondHearingDevice()?['sales_device_2']:[])];const listTotal=prefixes.reduce((sum,prefix)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[data-list-price-for="${prefix}_serial"]`)?.value)||0),0);const hearingSgk=prefixes.reduce((sum,prefix)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${prefix}_sgk"]`)?.value)||0),0);const hearingNet=prefixes.reduce((sum,prefix)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${prefix}_net_price"]`)?.value)||0),0);const netBeforeDiscount=Math.max(0,listTotal-hearingSgk);const difference=savedTotalDifference??(netBeforeDiscount-hearingNet);totalDiscountInput.title='İskonto / (Liste fiyatı − SGK)';if(!preserveDiscountInput&&document.activeElement!==totalDiscountInput)totalDiscountInput.value=netBeforeDiscount>0?'%'+(difference/netBeforeDiscount*100).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})+' - '+formatTurkishMoney(difference):'';}totalField.value=total>0?formatTurkishMoney(total):'';refreshSameModelSummary();if(salesGrandTotal)salesGrandTotal.textContent='Toplam Satış: '+formatTurkishMoney(total);const paymentType=detailsModal?.querySelector('[name="sales_payment_type"]'),paymentLocked=<?=json_encode($savedCashRecord !== [])?>;if(paymentType){if(total<=0)paymentType.value='';paymentType.disabled=paymentLocked||total<=0;paymentType.title=paymentLocked?'Gelir kaydı bulunduğu için ödeme şekli değiştirilemez.':(total<=0?'Ürün ve toplam tutar olmadan ödeme şekli seçilemez.':'');}};
  const clearSavedTotalDifference=event=>{const field=event.target;if(!(field instanceof HTMLInputElement||field instanceof HTMLSelectElement))return;if(/^(sales_device(?:_2)?_(?:sgk|net_price|discount_rate)|sales_total_discount_rate|sales_consumable_(?:quantity|price)|sales_charger_(?:net_price|price))$/.test(field.name)||field.closest('.sales-same-model-field')||event.type==='change'&&/^(sales_(?:brand|model|sale_date)|sales_device_2_(?:brand|model)|sales_(?:charger|consumable)_(?:promotion|stock_id))$/.test(field.name))savedTotalDifference=null;};
  detailsModal?.addEventListener('input',clearSavedTotalDifference,true);
  detailsModal?.addEventListener('change',clearSavedTotalDifference,true);
  totalDiscountInput?.addEventListener('focus',()=>totalDiscountInput.select());
  totalDiscountInput?.addEventListener('input',()=>{
    if(!totalDiscountModeActive||totalDiscountInput.readOnly)return;
    const devicePrefixes=['sales_device','sales_device_2'];
    const devices=devicePrefixes.map(prefix=>{
      const list=parseTurkishMoney(detailsModal?.querySelector(`[data-list-price-for="${prefix}_serial"]`)?.value)||0;
      const sgk=parseTurkishMoney(detailsModal?.querySelector(`[name="${prefix}_sgk"]`)?.value)||0;
      return {prefix,list,base:Math.max(0,list-sgk)};
    });
    const baseCents=devices.map(device=>Math.round(device.base*100));
    const availableCents=baseCents.reduce((sum,cents)=>sum+cents,0);
    const raw=totalDiscountInput.value.trim();
    const entered=parseTurkishMoney(raw)||0;
    const requested=raw.includes('%')?availableCents/100*entered/100:entered;
    const discountCents=Math.min(availableCents,Math.max(0,Math.round(requested*100)));
    const firstDiscount=availableCents>0?Math.round(discountCents*baseCents[0]/availableCents):0;
    const allocated=[firstDiscount,discountCents-firstDiscount];
    devices.forEach((device,index)=>{
      const net=detailsModal?.querySelector(`[name="${device.prefix}_net_price"]`);
      if(net)net.value=formatTurkishMoney((baseCents[index]-allocated[index])/100);
    });
    updateTotalAmount(true);
    persistDetails();
  });
  totalDiscountInput?.addEventListener('blur',()=>updateTotalAmount());
  const editableTotalField=detailsModal?.querySelector('[name="sales_payment_amount"]');
  editableTotalField?.addEventListener('change',()=>{
    if(detailsModal?.dataset.salesLocked==='1')return;
    const requested=parseTurkishMoney(editableTotalField.value);
    const chargerPromoted=detailsModal?.querySelector('[name="sales_charger_promotion"]')?.value==='Evet';
    const chargerTotal=chargerPromoted?0:(parseTurkishMoney(detailsModal?.querySelector('[name="sales_charger_net_price"]')?.value)||0);
    const consumablePrice=parseTurkishMoney(detailsModal?.querySelector('[name="sales_consumable_price"]')?.value)||0;
    const consumableQuantity=Number(detailsModal?.querySelector('[name="sales_consumable_quantity"]')?.value)||0;
    const consumableTotal=detailsModal?.querySelector('[name="sales_consumable_promotion"]')?.value==='Evet'||!detailsModal?.querySelector('[name="sales_consumable_stock_id"]')?.value?0:(parseTurkishMoney(detailsModal?.querySelector('[data-consumable-inline-total]')?.value)||consumablePrice*consumableQuantity);
    const otherTotal=chargerTotal+consumableTotal;
    const prefixes=['sales_device',...(hasSecondHearingDevice()?['sales_device_2']:[])].filter(prefix=>{
      const field=detailsModal?.querySelector(`[name="${prefix}_net_price"]`);
      return field&&!(prefix==='sales_device'&&deviceDetails?.hidden);
    });
    if(requested===null||requested<otherTotal||!prefixes.length){
      alert('Toplam tutar geçerli bir sayı olmalı ve diğer ürünlerin tutarından düşük olamaz.');
      updateTotalAmount();
      return;
    }
    const targetCents=Math.round((requested-otherTotal)*100);
    const weights=prefixes.map(prefix=>parseTurkishMoney(detailsModal?.querySelector(`[name="${prefix}_net_price"]`)?.value)||0);
    if(weights.every(weight=>weight<=0))prefixes.forEach((prefix,index)=>{weights[index]=Math.max(0,(parseTurkishMoney(detailsModal?.querySelector(`[data-list-price-for="${prefix}_serial"]`)?.value)||0)-(parseTurkishMoney(detailsModal?.querySelector(`[name="${prefix}_sgk"]`)?.value)||0));});
    const weightTotal=weights.reduce((sum,weight)=>sum+weight,0);
    let allocated=0;
    prefixes.forEach((prefix,index)=>{
      const cents=index===prefixes.length-1?targetCents-allocated:Math.round(targetCents*(weightTotal>0?weights[index]/weightTotal:1/prefixes.length));
      allocated+=cents;
      const netField=detailsModal.querySelector(`[name="${prefix}_net_price"]`);
      netField.value=formatTurkishMoney(cents/100);
      const discountField=detailsModal.querySelector(`[name="${prefix}_discount_rate"]`);
      const list=parseTurkishMoney(detailsModal.querySelector(`[data-list-price-for="${prefix}_serial"]`)?.value)||0;
      const sgk=parseTurkishMoney(detailsModal.querySelector(`[name="${prefix}_sgk"]`)?.value)||0;
      if(discountField)discountField.value=formatTurkishMoney(list-sgk-cents/100);
    });
    savedTotalDifference=null;
    updateTotalAmount();
    persistDetails();
  });
  ...
  detailsModal?.addEventListener('input',event=>{const field=event.target;if(!(field instanceof HTMLInputElement)||!/^sales_device(?:_2)?_net_price$/.test(field.name))return;updateTotalAmount();persistDetails();});
  ...
  const quantityLabel=consumableQuantityInput.closest('label');
  quantityLabel.style.setProperty('grid-column','1 / -1','important');
  consumableQuantityInput.style.setProperty('max-width','180px','important');
  consumableDetails.prepend(quantityLabel);
  const consumableItemsInput=consumableDetails.querySelector('[name="sales_consumable_items"]');
  const additionalConsumables=document.createElement('div');additionalConsumables.dataset.consumableRows='1';
  const consumableRowsStyle=document.createElement('style');
  consumableRowsStyle.textContent='#consumable-details [data-consumable-rows] label{display:flex;flex-direction:column;gap:2px;font:700 11px/15px Tahoma,"Segoe UI",sans-serif!important;color:#26384a}';document.head.append(consumableRowsStyle);
  additionalConsumables.style.cssText='grid-column:1/-1!important;display:grid;gap:6px';consumableDetails.append(additionalConsumables);
  const promotionLabel=consumablePromotionInput.closest('label');
  const promotionRow=document.createElement('div');promotionRow.className='sales-consumable-promotion-row';
  promotionRow.append(promotionLabel);consumableDetails.append(promotionRow);
  consumableRowsStyle.textContent+='#sales-details-modal #consumable-details .sales-consumable-promotion-row{grid-column:1/-1;display:flex;justify-content:center;margin-top:16px;padding-bottom:6px}#sales-details-modal #consumable-details .sales-consumable-promotion-row>label{display:flex;flex-direction:column;gap:2px;width:180px;max-width:100%;font:700 11px/15px Tahoma,"Segoe UI",sans-serif}';

  const syncConsumablePrice=()=>{
    const quantity=Math.max(1,Math.min(1000,Number(consumableQuantityInput.value)||1));
    consumableQuantityInput.max='1000';
    let previous=[];try{previous=JSON.parse(consumableItemsInput.value||'[]');}catch(_){}
    const rows=Array.from({length:quantity},(_,index)=>{
      const stockId=index===0?consumableSelect.value:(previous[index]?.stock_id||consumableSelect.value);
      const stock=consumableStocks.find(item=>String(item.id)===String(stockId));
      const price=listPriceForStock(stock)||formatTurkishMoney(stock?.sale_price||0);
      return {stock_id:String(stockId||''),price,unit:stock?.unit||'Adet'};
    });
    consumableItemsInput.value=JSON.stringify(rows);
    consumablePriceInput.value=rows[0].price;consumableUnitInput.value=rows[0].unit;
    consumableDescriptionInput.value=consumableSelect.selectedOptions[0]?.textContent||'';
    consumableInlineTotal.value=formatTurkishMoney(rows.reduce((total,row)=>total+(parseTurkishMoney(row.price)||0),0));
    while(additionalConsumables.children.length>quantity-1)additionalConsumables.lastElementChild.remove();
    rows.slice(1).forEach((row,offset)=>{
      ...
      if(!line){
        ...
        const price=document.createElement('label');price.textContent='Satış Fiyatı';const priceInput=document.createElement('input');priceInput.readOnly=true;price.append(priceInput);
        line.append(label,unit,price);additionalConsumables.append(line);
        select.addEventListener('change',()=>{const items=JSON.parse(consumableItemsInput.value);items[index].stock_id=select.value;consumableItemsInput.value=JSON.stringify(items);syncConsumablePrice();});
      }
      line.querySelector('select').value=row.stock_id;const inputs=line.querySelectorAll('input');inputs[0].value=row.unit;inputs[1].value=row.price;
    });
    updateTotalAmount();
  };
  ...
  const syncConsumableModal=()=>{const stock=consumableStocks.find(item=>String(item.id)===String(modalConsumableSelect.value)),isPromotion=modalConsumablePromotion.value==='Evet',price=listPriceForStock(stock)||formatTurkishMoney(stock?.sale_price||0);if(modalConsumablePrice)modalConsumablePrice.value=price;if(modalConsumableUnit)modalConsumableUnit.value=stock?.unit||'Adet';if(modalConsumableDescription)modalConsumableDescription.value=stock?`${stock.stock_code} — ${stock.stock_name}`:'';const amount=parseTurkishMoney(modalConsumablePrice?.value)||0,quantity=Math.max(1,Number(modalConsumableQuantity?.value)||1);if(modalConsumableTotal)modalConsumableTotal.value=formatTurkishMoney(isPromotion?0:amount*quantity);};
  ...
  chargerDetails.querySelector('[name="sales_charger_promotion"]').addEventListener('change',()=>{
    ...
    if(!promoted){
      const stock=chargerDeviceStocks.find(item=>item.brand===chargerBrandSelect.value&&item.model===chargerModelSelect.value);
      chargerPriceInput.value=stock?(listPriceForStock(stock)||formatTurkishMoney(stock.sale_price||0)):'';
      chargerDiscountInput.value='';
      chargerDetails.querySelector('[name="sales_charger_sgk"]').value='';
      applyDiscount(chargerPriceInput,chargerDiscountInput,chargerNetPriceInput);
    }
    updateTotalAmount();
  });
  ...
  if(cashSourceForm){
    ...
    savedCashRecords.forEach(record=>record.payment_type=normalizeCashPaymentType(record.payment_type));
    if(savedCashRecord?.id)savedCashRecord.payment_type=normalizeCashPaymentType(savedCashRecord.payment_type);
    ...
    if(savedCashRecord&&savedCashRecord.transaction_date){
      ...
      const savedPaymentField=cashSourceForm.querySelector('[name="payment_type"]');if(savedPaymentField)savedPaymentField.value=savedCashRecord.payment_type||'';
    ...
    if(persistedUpdateButton){
      ...
      cleanUpdateButton.addEventListener('click',()=>{
        ...
        const values={
          ...
          cash_update_payment_type:cashSourceForm.querySelector('[name="payment_type"]')?.value||'',
          cash_update_installment_count:cashSourceForm.querySelector('[name="installment_count"]')?.value||'1',
          cash_update_current_account_id:cashSourceForm.querySelector('[name="current_account_id"]')?.value||'',cash_update_bank_name:cashSourceForm.querySelector('[name="bank_name"]')?.value||'',
          cash_update_commission_rate:cashSourceForm.querySelector('[name="commission_rate"]')?.value||'',
          cash_update_extra_id:extraRecordId,cash_update_extra_date:extra?.querySelector('[name="extra_transaction_date"]')?.value||'',cash_update_extra_description:extra?.querySelector('[name="extra_description"]')?.value||'',
  ...
  if(cashPaymentSelect&&cashPaymentLabel){if(cashPaymentLabel.firstChild?.nodeType===Node.TEXT_NODE)cashPaymentLabel.firstChild.nodeValue='Ödeme Şekli';const installmentLabel=document.createElement('label');installmentLabel.textContent='KK Taksit Sayısı';const installments=document.createElement('input');installments.type='number';installments.name='installment_count';installments.min='1';installments.step='1';installments.value='1';installmentLabel.append(installments);cashPaymentLabel.after(installmentLabel);}
  ...
  paymentSelect?.addEventListener('change',()=>{if(!(window.__savedCashRecords||[])[0]?.id&&cashPaymentSelect){cashPaymentSelect.value=normalizeCashPaymentType(paymentSelect.value);cashPaymentSelect.dispatchEvent(new Event('change',{bubbles:true}));}});
  ...
  const repairIncomeLayouts=()=>{const primary=cashSourceForm?.querySelector('section'),primaryPaymentType=cashSourceForm?.dataset.forcedPaymentType||cashPaymentSelect?.value||salesPaymentToCashType(paymentSelect?.value||'')||'cash';if(primary){forceLabel(primary,'transaction_date','1 / 2','1');forceLabel(primary,'payment_type','2 / 3','1');forceLabel(primary,'bank_name','1 / 2','2',['eft_transfer','credit_card','mail_order'].includes(primaryPaymentType));forceLabel(primary,'current_account_id','2 / 3','2',primaryPaymentType==='mail_order');forceLabel(primary,'installment_count',primaryPaymentType==='term'?'1 / 2':'2 / 3','2',['credit_card','term'].includes(primaryPaymentType));forceLabel(primary,'commission_rate','2 / 3','3',primaryPaymentType==='credit_card');forceLabel(primary,'amount',primaryPaymentType==='credit_card'?'1 / 2':'1 / -1','3');forceLabel(primary,'description','1 / -1','4');}if(primary?.querySelector('[data-primary-term-schedule]')){primary.style.setProperty('display','grid','important');primary.style.setProperty('grid-template-columns','repeat(2,minmax(0,1fr))','important');forceLabel(primary,'installment_count','1 / 2','2');forceLabel(primary,'amount','2 / 3','2');forceLabel(primary,'commission_rate','1 / 2','3',false);const schedule=primary.querySelector('[data-primary-term-schedule]');schedule.style.setProperty('grid-column','1 / -1','important');schedule.style.setProperty('grid-row','3','important');forceLabel(primary,'description','1 / -1','4');}else if(primary&&primaryPaymentType==='credit_card'){primary.style.setProperty('display','grid','important');primary.style.setProperty('grid-template-columns','repeat(2,minmax(0,1fr))','important');forceLabel(primary,'bank_name','1 / 2','2');forceLabel(primary,'installment_count','2 / 3','2');forceLabel(primary,'amount','1 / 2','3');forceLabel(primary,'commission_rate','2 / 3','3');forceLabel(primary,'description','1 / -1','4');}const extra=cashSourceForm?.querySelector('[data-extra-income]');if(!extra)return;extra.style.setProperty('display','grid','important');extra.style.setProperty('grid-template-columns','repeat(2,minmax(0,1fr))','important');extra.querySelectorAll('input,select,textarea').forEach(field=>field.style.setProperty('box-sizing','border-box','important'));const type=extra.querySelector('[name="extra_payment_type"]')?.value||'cash';if(type!=='term')extra.querySelector('[data-term-schedule]')?.remove();forceLabel(extra,'extra_transaction_date','1 / 2','2');forceLabel(extra,'extra_payment_type','2 / 3','2');if(type==='credit_card'){forceLabel(extra,'extra_bank_name','1 / 2','3');forceLabel(extra,'extra_current_account_id','2 / 3','3',false);forceLabel(extra,'extra_installment_count','2 / 3','3');forceLabel(extra,'extra_commission_rate','2 / 3','4');forceLabel(extra,'extra_amount','1 / 2','4');forceLabel(extra,'extra_description','1 / -1','5');}else if(type==='mail_order'||type==='eft_transfer'){forceLabel(extra,'extra_bank_name','1 / 2','3');forceLabel(extra,'extra_current_account_id','2 / 3','3',type==='mail_order');forceLabel(extra,'extra_installment_count','1 / 2','4',false);forceLabel(extra,'extra_commission_rate','2 / 3','4',false);forceLabel(extra,'extra_amount','1 / -1','4');forceLabel(extra,'extra_description','1 / -1','5');}else if(type==='term'){forceLabel(extra,'extra_bank_name','1 / 2','3',false);forceLabel(extra,'extra_current_account_id','2 / 3','3',false);forceLabel(extra,'extra_installment_count','1 / 2','3');forceLabel(extra,'extra_commission_rate','2 / 3','3',false);forceLabel(extra,'extra_amount','1 / -1','4',false);forceLabel(extra,'extra_description','1 / -1','5');}else if(type==='cash'){forceLabel(extra,'extra_bank_name','1 / 2','3',false);forceLabel(extra,'extra_current_account_id','2 / 3','3',false);forceLabel(extra,'extra_installment_count','1 / 2','3',false);forceLabel(extra,'extra_commission_rate','2 / 3','3',false);forceLabel(extra,'extra_amount','1 / -1','3');forceLabel(extra,'extra_description','1 / -1','4');}};new MutationObserver(repairIncomeLayouts).observe(cashSourceForm||document.body,{childList:true,subtree:true});cashSourceForm?.addEventListener('change',()=>setTimeout(repairIncomeLayouts,0));setTimeout(repairIncomeLayouts,0);
  ...
  const alignPrimaryTermSummary=()=>{const section=cashSourceForm?.querySelector('section'),schedule=section?.querySelector('[data-primary-term-schedule]');if(!section||!schedule)return;const placeLabel=(name,column,row)=>{const label=section.querySelector(`[name="${name}"]`)?.closest('label');if(!label)return;label.style.setProperty('display','flex','important');label.style.setProperty('grid-column',column,'important');label.style.setProperty('grid-row',row,'important');};section.style.setProperty('grid-template-columns','repeat(2,minmax(0,1fr))','important');placeLabel('transaction_date','1 / 2','1');placeLabel('payment_type','2 / 3','1');placeLabel('installment_count','1 / 2','2');placeLabel('amount','2 / 3','2');schedule.style.setProperty('display','grid','important');schedule.style.setProperty('grid-column','1 / -1','important');schedule.style.setProperty('grid-row','3','important');placeLabel('description','1 / -1','4');};const queuePrimaryTermSummary=()=>[0,80,220].forEach(delay=>setTimeout(alignPrimaryTermSummary,delay));new MutationObserver(queuePrimaryTermSummary).observe(cashSourceForm||document.body,{childList:true,subtree:true});cashSourceForm?.addEventListener('change',queuePrimaryTermSummary);cashPaymentSelect?.addEventListener('change',queuePrimaryTermSummary);paymentSelect?.addEventListener('change',queuePrimaryTermSummary);cashIconLink?.addEventListener('click',queuePrimaryTermSummary);queuePrimaryTermSummary();
  ...
  const sideToggleFor=name=>detailsModal?.querySelector(`.sales-hearing-side-toggle[data-side-field="${name}"]`);
  ...
  const addProductCancel=container=>{const isDevice=container.classList.contains('sales-device-details');container.classList.remove('has-product-delete');container.querySelectorAll('.sales-product-cancel').forEach(button=>button.remove());container.querySelectorAll('.sales-serial-delete-row').forEach(row=>row.replaceWith(...row.childNodes));if(isDevice&&!container.querySelector('.sales-product-section-heading')){
    const second=container.id==='hearing-device-details-2',fieldName=second?'sales_device_2_side':'sales_device_side';
    const heading=document.createElement('strong');heading.className='sales-product-section-heading';heading.textContent=second?'İşitme Cihazı 2':'İşitme Cihazı 1';
    const side=document.createElement('input');side.type='hidden';side.name=fieldName;side.value=savedSaleProducts[fieldName]==='sol'?'sol':(savedSaleProducts[fieldName]==='sağ'?'sağ':(second?'sol':'sağ'));
    const toggle=document.createElement('button');toggle.type='button';toggle.className='sales-hearing-side-toggle';toggle.dataset.sideField=fieldName;toggle.title='Kulak yönünü değiştir';
    const ear=document.createElement('i');ear.className='icon-base ti tabler-ear';ear.setAttribute('aria-hidden','true');
    const label=document.createElement('span');
    const renderSide=()=>{label.textContent=side.value==='sol'?'Sol':'Sağ';toggle.dataset.side=side.value;toggle.setAttribute('aria-label',(second?'İşitme Cihazı 2':'İşitme Cihazı 1')+' kulak: '+label.textContent+'. Yönü değiştir');detailsModal?.querySelector(`[name="${second?'sales_device_2_serial':'sales_device_serial'}"]`)?.setAttribute('aria-label','Seri No ('+label.textContent+')');ear.style.transform=side.value==='sol'?'scaleX(-1)':'';};
    const opposite=value=>value==='sağ'?'sol':'sağ';
    const peer=detailsModal?.querySelector(second?'#hearing-device-details [name="sales_device_side"]':'#hearing-device-details-2 [name="sales_device_2_side"]');
    if(peer&&peer.value===side.value){side.value=second?opposite(peer.value):side.value;if(!second)peer.value=opposite(side.value);sideToggleFor(peer.name)?.dispatchEvent(new Event('hearing-side-sync'));}
    toggle.addEventListener('hearing-side-sync',renderSide);
    toggle.append(ear,label);toggle.addEventListener('click',()=>{side.value=opposite(side.value);renderSide();const other=detailsModal?.querySelector(second?'#hearing-device-details [name="sales_device_side"]':'#hearing-device-details-2 [name="sales_device_2_side"]');if(other){other.value=opposite(side.value);sideToggleFor(other.name)?.dispatchEvent(new Event('hearing-side-sync'));}persistDetails();});renderSide();
    heading.append(toggle,side);container.prepend(heading);
  }return container;};
  ...
  const addExtraDevice=number=>{
    ...
    const listPriceLabel=document.createElement('label'),listPriceInput=document.createElement('input');
    listPriceLabel.append('Liste Fiyatı');listPriceInput.type='text';listPriceInput.readOnly=true;
    listPriceInput.className='sales-list-price';listPriceInput.dataset.listPriceFor=`sales_device_${number}_serial`;
    listPriceLabel.append(listPriceInput);serial.closest('label').after(listPriceLabel);
    const refreshListPrice=()=>{listPriceInput.value=listPriceForStock(hearingDeviceStocks.find(stock=>stock.brand===brand.value&&stock.model===model.value));};
    brand.addEventListener('change',()=>{listPriceInput.value='';});
    model.addEventListener('change',refreshListPrice);
    salesDateInput?.addEventListener('change',refreshListPrice);
  ...
  const hasSavedFirstDevice=['sales_brand','sales_model','sales_device_serial'].some(name=>String(savedSaleProducts[name]??'').trim()!=='');
  const hasSavedSecondDevice=['sales_device_2_brand','sales_device_2_model','sales_device_2_serial'].some(name=>String(savedSaleProducts[name]??'').trim()!=='');
  const hasSavedOtherProduct=['sales_charger_brand','sales_charger_model','sales_charger_serial','sales_consumable_stock_id'].some(name=>String(savedSaleProducts[name]??'').trim()!=='');
  const savedDevicesMatch=hasSavedFirstDevice&&hasSavedSecondDevice&&savedSaleProducts.sales_brand===savedSaleProducts.sales_device_2_brand&&savedSaleProducts.sales_model===savedSaleProducts.sales_device_2_model;
  const initialPairMode=['same','separate'].includes(savedSaleProducts.sales_hearing_pair_mode)?savedSaleProducts.sales_hearing_pair_mode:(savedDevicesMatch||!hasSavedFirstDevice&&!hasSavedSecondDevice&&!hasSavedOtherProduct?'same':'separate');
  detailsModal.dataset.hearingPair=initialPairMode;
  const makeSameModelField=(caption,editable=false)=>{const label=document.createElement('label'),input=document.createElement('input');label.className='sales-same-model-field';label.append(caption);input.type='text';input.readOnly=!editable;if(editable)input.inputMode='decimal';label.append(input);deviceDetails?.append(label);return input;};
  const sameQuantity=makeSameModelField('Adet');sameQuantity.value='2';
  const sameListTotal=makeSameModelField('Liste Fiyatı (2 Adet)');sameListTotal.closest('label')?.classList.add('sales-same-model-list-price');
  const sameSgkTotal=makeSameModelField('SGK Toplamı',true);
  const sameNetTotal=makeSameModelField('Net Fiyat Toplamı');
  const sameSerialLabels=()=>({first:deviceSerialInput?.closest('label'),second:detailsModal?.querySelector('[name="sales_device_2_serial"]')?.closest('label')});
  const placeSerialSideToggle=(label,fieldName,inSerialLabel)=>{
    if(!label)return;
    const toggle=sideToggleFor(fieldName),heading=detailsModal?.querySelector(`[name="${fieldName}"]`)?.closest('.sales-product-section-heading');
    if(!toggle||!heading)return;
    let caption=label.querySelector(':scope > .sales-serial-caption');
    if(inSerialLabel){
      if(!caption){caption=document.createElement('span');caption.className='sales-serial-caption';const textNode=label.firstChild;if(textNode?.nodeType===Node.TEXT_NODE)textNode.replaceWith(caption);else label.prepend(caption);}
      caption.replaceChildren('Seri No',toggle);
    }else{
      heading.insertBefore(toggle,heading.querySelector(`[name="${fieldName}"]`));
      if(caption)caption.replaceWith(document.createTextNode('Seri No'));
      else if(label.firstChild?.nodeType===Node.TEXT_NODE)label.firstChild.nodeValue='Seri No';
    }
  };
  refreshSameModelSummary=()=>{
    if(detailsModal?.dataset.hearingPair!=='same')return;
    const unitList=parseTurkishMoney(deviceListPriceInput?.value)||0;
    const sgk=['sales_device_sgk','sales_device_2_sgk'].reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);
    const netBeforeDiscount=Math.max(0,unitList*2-sgk);
    sameListTotal.value=unitList?formatTurkishMoney(unitList*2):'';
    if(document.activeElement!==sameSgkTotal)sameSgkTotal.value=formatTurkishMoney(sgk);
    sameNetTotal.value=formatTurkishMoney(netBeforeDiscount);
    sameSgkTotal.readOnly=detailsModal?.dataset.salesLocked==='1';
  };
  const syncSameModel=()=>{
    if(detailsModal.dataset.hearingPair!=='same')return;
    const second=detailsModal?.querySelector('#hearing-device-details-2');if(!second)return;
    const brand=second.querySelector('[name="sales_device_2_brand"]'),model=second.querySelector('[name="sales_device_2_model"]'),serial=detailsModal.querySelector('[name="sales_device_2_serial"]');
    if(brand.value!==brandSelect.value){brand.value=brandSelect.value;serial.value='';brand.dispatchEvent(new Event('change',{bubbles:true}));}
    if(model.value!==modelSelect.value){if(modelSelect.value&&![...model.options].some(option=>option.value===modelSelect.value))model.add(new Option(modelSelect.value,modelSelect.value));model.value=modelSelect.value;serial.value='';model.dispatchEvent(new Event('change',{bubbles:true}));}
    if(!second.querySelector('[name="sales_device_2_sgk"]').value)second.querySelector('[name="sales_device_2_sgk"]').value=detailsModal?.querySelector('[name="sales_device_sgk"]')?.value||'';
    hideSelectedSerials();updateTotalAmount();
  };
  const pairDeviceFields=['sales_brand','sales_model','sales_device_serial','sales_device_sgk','sales_device_discount_rate','sales_device_net_price','sales_device_2_brand','sales_device_2_model','sales_device_2_serial','sales_device_2_sgk','sales_device_2_discount_rate','sales_device_2_net_price'];
  const pairSummaryFields=['sales_sale_date','sales_warranty_start','sales_warranty_end','sales_invoice_no','sales_payment_type','sales_total_sgk','sales_total_discount_rate','sales_payment_amount'];
  const captureSameModelCard=()=>{
    const fields={},summary={};
    pairDeviceFields.forEach(name=>{fields[name]=detailsModal.querySelector(`[name="${name}"]`)?.value||'';});
    pairSummaryFields.forEach(name=>{summary[name]=detailsModal.querySelector(`[name="${name}"]`)?.value||'';});
    return {fields,summary,firstList:deviceListPriceInput?.value||'',secondList:detailsModal.querySelector('[data-list-price-for="sales_device_2_serial"]')?.value||'',firstBase:detailsModal.querySelector('[name="sales_device_net_price"]')?.dataset.listPrice||'',secondBase:detailsModal.querySelector('[name="sales_device_2_net_price"]')?.dataset.listPrice||'',difference:savedTotalDifference};
  };
  const showSameModelCardFields=show=>{
    const snapshot=show?sameModelCardSnapshot:null;
    pairDeviceFields.forEach(name=>{
      const field=detailsModal.querySelector(`[name="${name}"]`);if(!field)return;
      const fieldValue=snapshot?.fields[name]||'';
      if(field instanceof HTMLSelectElement&&fieldValue&&![...field.options].some(option=>option.value===fieldValue))field.add(new Option(fieldValue,fieldValue));
      field.value=fieldValue;
    });
    pairSummaryFields.forEach(name=>{const field=detailsModal.querySelector(`[name="${name}"]`);if(field)field.value=snapshot?.summary[name]||'';});
    if(deviceListPriceInput)deviceListPriceInput.value=snapshot?.firstList||'';
    const secondList=detailsModal.querySelector('[data-list-price-for="sales_device_2_serial"]');if(secondList)secondList.value=snapshot?.secondList||'';
    [['sales_device_net_price',snapshot?.firstBase],['sales_device_2_net_price',snapshot?.secondBase]].forEach(([name,base])=>{const field=detailsModal.querySelector(`[name="${name}"]`);if(!field)return;if(base)field.dataset.listPrice=base;else delete field.dataset.listPrice;});
    savedTotalDifference=snapshot?.difference??null;
  };
  const applyPairMode=(mode,initial=false)=>{
    if(detailsModal?.dataset.salesLocked==='1'&&!initial)return;
    const switchingSameModelCard=sameModelOnlyCard&&!initial&&mode!==detailsModal.dataset.hearingPair;
    if(switchingSameModelCard&&detailsModal.dataset.hearingPair==='same')sameModelCardSnapshot=captureSameModelCard();
    const same=mode==='same';
    if(same){setProductType('İşitme Cihazı');if(deviceDetails?.hidden)showFirstDevice();if(!detailsModal?.querySelector('#hearing-device-details-2'))addExtraDevice(2);}
    detailsModal.dataset.hearingPair=same?'same':'separate';
    const second=detailsModal.querySelector('#hearing-device-details-2'),labels=sameSerialLabels();
    if(same&&labels.first&&labels.second){labels.first.after(labels.second);placeSerialSideToggle(labels.first,'sales_device_side',true);placeSerialSideToggle(labels.second,'sales_device_2_side',true);detailsModal.querySelectorAll('.sales-hearing-side-toggle').forEach(toggle=>toggle.dispatchEvent(new Event('hearing-side-sync')));syncSameModel();}
    if(!same&&labels.first&&labels.second){placeSerialSideToggle(labels.first,'sales_device_side',false);placeSerialSideToggle(labels.second,'sales_device_2_side',false);second?.querySelector('[name="sales_device_2_model"]')?.closest('label')?.after(labels.second);}
    deviceDetails?.querySelector('.sales-product-section-heading')?.firstChild&&(deviceDetails.querySelector('.sales-product-section-heading').firstChild.nodeValue=same?'İşitme Cihazı':'İşitme Cihazı 1');
    const modelLabel=modelSelect?.closest('label');if(same&&modelLabel){modelLabel.after(sameQuantity.closest('label'));labels.second?.after(sameListTotal.closest('label'));sameListTotal.closest('label')?.after(sameSgkTotal.closest('label'),sameNetTotal.closest('label'));}
    if(switchingSameModelCard)showSameModelCardFields(same);
    detailsModal.querySelectorAll('[data-hearing-pair]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.hearingPair===detailsModal.dataset.hearingPair)));
    arrangeProductSections();updateDeviceAddButton();updateTotalAmount();if(!initial)persistDetails();
  };
  detailsModal?.addEventListener('click',event=>{const button=event.target.closest('[data-hearing-pair]');if(button)applyPairMode(button.dataset.hearingPair);});
  detailsModal?.addEventListener('change',event=>{
    if(detailsModal.dataset.hearingPair!=='same'||![brandSelect,modelSelect].includes(event.target))return;
    if(event.target===modelSelect){
      deviceSerialInput.value='';
      const stocks=hearingDeviceStocks.filter(stock=>stock.brand===brandSelect.value&&stock.model===modelSelect.value);
      fillSerialOptions(deviceSerialInput,[...stocks,...invoiceMatchedSerials(brandSelect.value,modelSelect.value)]);
    }
    queueMicrotask(syncSameModel);
  });
  sameSgkTotal.addEventListener('input',()=>{
    if(detailsModal.dataset.hearingPair!=='same'||sameSgkTotal.readOnly)return;
    const oldList=(parseTurkishMoney(deviceListPriceInput?.value)||0)*2;
    const oldSgk=['sales_device_sgk','sales_device_2_sgk'].reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);
    const oldNet=['sales_device_net_price','sales_device_2_net_price'].reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);
    const discount=Math.max(0,oldList-oldSgk-oldNet),sgkCents=Math.max(0,Math.min(Math.round(oldList*100),Math.round((parseTurkishMoney(sameSgkTotal.value)||0)*100)));
    const sgkParts=[Math.floor(sgkCents/2),sgkCents-Math.floor(sgkCents/2)];
    const netCents=Math.max(0,Math.round(oldList*100)-sgkCents-Math.round(discount*100));
    const netParts=[Math.floor(netCents/2),netCents-Math.floor(netCents/2)];
    ['sales_device','sales_device_2'].forEach((prefix,index)=>{detailsModal.querySelector(`[name="${prefix}_sgk"]`).value=formatTurkishMoney(sgkParts[index]/100);detailsModal.querySelector(`[name="${prefix}_net_price"]`).value=formatTurkishMoney(netParts[index]/100);});
    updateTotalAmount();persistDetails();
  });
  ...
  setTimeout(()=>{try{const saved=JSON.parse(details?.value||'{}');for(let number=1;number<=2;number++){const isFirst=number===1,device=isFirst?deviceDetails:detailsModal?.querySelector('#hearing-device-details-2'),brand=isFirst?brandSelect:device?.querySelector('[name="sales_device_2_brand"]'),model=isFirst?modelSelect:device?.querySelector('[name="sales_device_2_model"]'),serial=isFirst?deviceSerialInput:detailsModal?.querySelector('[name="sales_device_2_serial"]'),sgk=isFirst?detailsModal?.querySelector('[name="sales_device_sgk"]'):device?.querySelector('[name="sales_device_2_sgk"]'),savedBrand=saved[isFirst?'sales_brand':'sales_device_2_brand']||'',savedModel=saved[isFirst?'sales_model':'sales_device_2_model']||'',savedSerial=saved[isFirst?'sales_device_serial':'sales_device_2_serial']||'';if(!device||!brand||!model||!serial||!savedBrand||!savedModel)continue;brand.value=savedBrand;if(![...model.options].some(option=>option.value===savedModel))model.add(new Option(savedModel,savedModel));model.disabled=false;model.value=savedModel;const stocks=hearingDeviceStocks.filter(item=>item.brand===savedBrand&&item.model===savedModel);fillSerialOptions(serial,[...stocks,...invoiceMatchedSerials(savedBrand,savedModel)]);if(savedSerial&&![...serial.options].some(option=>option.value===savedSerial))serial.add(new Option(savedSerial,savedSerial));serial.value=savedSerial;if(sgk)sgk.value=saved[isFirst?'sales_device_sgk':'sales_device_2_sgk']||sgk.value;}}catch(_){}},0);
  // Restore agreed amounts after initial model selection calculated default prices.
  ['sales_device_sgk','sales_device_discount_rate','sales_device_net_price',
   'sales_device_2_sgk','sales_device_2_discount_rate','sales_device_2_net_price',
   'sales_charger_sgk','sales_charger_discount_rate','sales_charger_net_price'].forEach(name=>{
    const field=detailsModal?.querySelector(`[name="${name}"]`);
    if(field&&Object.prototype.hasOwnProperty.call(savedSaleProducts,name))field.value=savedSaleProducts[name]??'';
  });
  // Kayıtlı toplam net fiyatlardan farklıysa farkı bilgi amaçlı iskontoda göster;
  // net fiyatları ve bunlardan hesaplanan satış toplamını değiştirme.
  const savedTotalRaw=String(savedSaleProducts.sales_payment_amount??'').trim();
  const savedSaleTotal=savedTotalRaw===''?null:parseTurkishMoney(savedTotalRaw);
  if(savedSaleTotal!==null&&savedSaleTotal>0&&hasSecondHearingDevice()){
    const deviceNet=['sales_device_net_price','sales_device_2_net_price'].reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal.querySelector(`[name="${name}"]`)?.value)||0),0);
    const chargerNet=detailsModal.querySelector('[name="sales_charger_promotion"]')?.value==='Evet'?0:(parseTurkishMoney(detailsModal.querySelector('[name="sales_charger_net_price"]')?.value)||0);
    const consumableTotal=detailsModal.querySelector('[data-consumable-inline-total]')?.value;
    const consumableNet=detailsModal.querySelector('[name="sales_consumable_promotion"]')?.value==='Evet'||!detailsModal.querySelector('[name="sales_consumable_stock_id"]')?.value?0:(consumableTotal?(parseTurkishMoney(consumableTotal)||0):(parseTurkishMoney(detailsModal.querySelector('[name="sales_consumable_price"]')?.value)||0)*(Number(detailsModal.querySelector('[name="sales_consumable_quantity"]')?.value)||0));
    const gap=Math.round((deviceNet+chargerNet+consumableNet-savedSaleTotal)*100)/100;
    if(gap>0)savedTotalDifference=gap;
    else{
      const savedDiscountText=String(savedSaleProducts.sales_total_discount_rate??'').trim();
      const savedDiscount=parseTurkishMoney(savedDiscountText.includes(' - ')?savedDiscountText.split(' - ').pop():savedDiscountText);
      if(savedDiscountText!==''&&savedDiscount>0)savedTotalDifference=savedDiscount;
    }
  }
  [80,250,600].forEach(delay=>setTimeout(()=>{try{if(sameModelOnlyCard&&detailsModal?.dataset.hearingPair==='separate')return;const saved=JSON.parse(details?.value||'{}');if(detailsModal?.dataset.hearingPair==='same'&&(brandSelect?.value!==saved.sales_brand||modelSelect?.value!==saved.sales_model))return;[[deviceSerialInput,saved.sales_device_serial],[detailsModal?.querySelector('[name="sales_device_2_serial"]'),saved.sales_device_2_serial]].forEach(([serial,savedSerial])=>{if(!serial||!savedSerial)return;if(![...serial.options].some(option=>option.value===savedSerial))serial.add(new Option(savedSerial,savedSerial));serial.value=savedSerial;});}catch(_){}updateTotalAmount();},delay));
  ...
  if(initialPairMode==='same')applyPairMode('same',true);else detailsModal.dataset.hearingPair='separate';
...
#sales-details-modal .sales-hearing-side-toggle{display:inline-flex!important;align-items:center!important;gap:4px!important;margin-left:6px!important;padding:0 3px!important;border:0!important;background:transparent!important;color:#159447!important;font:700 12px Tahoma,"Segoe UI",sans-serif!important;cursor:pointer!important}
#sales-details-modal .sales-hearing-side-toggle .ti{display:inline-block!important;width:15px!important;height:15px!important;font-size:15px!important;color:#159447!important}
#sales-details-modal .sales-hearing-side-toggle[data-side="sol"],#sales-details-modal .sales-hearing-side-toggle[data-side="sol"] .ti{color:#1874b9!important}
#sales-details-modal .sales-serial-caption{display:inline-flex!important;align-items:center!important;gap:4px!important;min-height:15px!important}
#sales-details-modal .sales-serial-caption .sales-hearing-side-toggle{height:16px!important;min-height:16px!important;max-height:16px!important;margin-left:0!important;padding:0 2px!important;font-size:11px!important;line-height:16px!important}
...
document.addEventListener('DOMContentLoaded',()=>{
  const normalizeIncomeFooter=()=>{
    ...
    const footerButtons=[...footer.querySelectorAll('button')],addButton=footer.querySelector('[aria-label="Bir gelir kaydı daha ekle"]'),actionButtons=footerButtons.filter(button=>button!==addButton&&!button.matches('[data-cancel-last-income]'));
    ...
    [...footer.querySelectorAll('button')].forEach(button=>{
      if(button.matches('[data-cash-close],[data-cancel-last-income],[aria-label="Bir gelir kaydı daha ekle"]'))return;
  ...
  const lockIncomeFooterSize=()=>document.querySelectorAll('form[action*="cash.php"].repair-dialog:not(.classic-cash-window-dialog)>footer button:not([data-cancel-last-income])').forEach(button=>{
...
window.addEventListener('click',event=>{
  ...
  if(!button||button.matches('[data-cash-close],[data-cancel-last-income],[aria-label="Bir gelir kaydı daha ekle"]'))return;
...
window.addEventListener('click',async event=>{
  ...
  set('cash_update_current_account_id',form.querySelector('[name="current_account_id"]')?.value||'');
  ...
  set('cash_update_extra_date',extra?.querySelector('[name="extra_transaction_date"]')?.value||'');
...
<style>#vox-sales-confirm{position:fixed;inset:0;z-index:5001;display:grid;place-items:center;padding:24px;background:rgba(26,28,36,.62)}#vox-sales-confirm[hidden]{display:none}#vox-sales-confirm-panel{width:min(440px,calc(100vw - 48px));padding:28px;border-radius:12px;background:#fff;box-shadow:0 20px 50px rgba(0,0,0,.28);color:#2d2f43}#vox-sales-confirm-message{margin:0;line-height:1.55}#vox-sales-confirm-actions{display:flex;justify-content:center;gap:10px;margin-top:24px}#vox-sales-confirm-actions button{min-width:96px;height:42px;border:0;border-radius:7px;font:inherit;font-weight:700;cursor:pointer}#vox-sales-confirm-ok{background:#e04f55;color:#fff}#vox-sales-confirm-cancel{background:#eef0f5;color:#2d2f43}</style>
<script>(()=>{window.voxConfirm=(message,options={})=>new Promise(resolve=>{let dialog=document.getElementById('vox-sales-confirm');if(!dialog){dialog=document.createElement('div');dialog.id='vox-sales-confirm';dialog.hidden=true;document.body.append(dialog);}if(!dialog.querySelector('#vox-sales-confirm-message')||!dialog.querySelector('#vox-sales-confirm-cancel')||!dialog.querySelector('#vox-sales-confirm-ok')){dialog.innerHTML='<section id="vox-sales-confirm-panel" role="dialog" aria-modal="true"><p id="vox-sales-confirm-message"></p><div id="vox-sales-confirm-actions"><button type="button" id="vox-sales-confirm-cancel" data-vox-confirm-cancel>İptal</button><button type="button" id="vox-sales-confirm-ok">Sil</button></div></section>';dialog.querySelector('#vox-sales-confirm-cancel').addEventListener('click',()=>{dialog.hidden=true;dialog._resolve?.(false);});dialog.querySelector('#vox-sales-confirm-ok').addEventListener('click',()=>{dialog.hidden=true;dialog._resolve?.(true);});}dialog.querySelector('#vox-sales-confirm-message').textContent=String(message||'');dialog.querySelector('#vox-sales-confirm-cancel').textContent=options.cancelLabel||'İptal';dialog.querySelector('#vox-sales-confirm-ok').textContent=options.confirmLabel||'Sil';dialog._resolve=resolve;dialog.hidden=false;dialog.querySelector('#vox-sales-confirm-cancel').focus();});document.addEventListener('click',event=>{const button=event.target.closest('[aria-label="İkinci gelir kaydını sil"]');if(!button)return;event.preventDefault();event.stopImmediatePropagation();window.voxConfirm('Bu ikinci gelir kaydını silmek istiyor musunuz?').then(approved=>{const record=(window.__savedCashRecords||[])[1];if(!approved||!record?.id)return;const form=document.createElement('form');form.method='post';form.action=<?=json_encode(url('patient-followup.php?id='.$id))?>;for(const [name,value] of Object.entries({csrf:<?=json_encode(csrf())?>,action:'cash_delete_only',edit_id:<?=json_encode((string)$editId)?>,cash_delete_id:record.id})){const input=document.createElement('input');input.type='hidden';input.name=name;input.value=value;form.append(input);}document.body.append(form);form.submit();});},true);})();</script>
<script>
document.addEventListener('DOMContentLoaded',()=>{
...
#sales-details-modal .classic-sales-section-title[data-sales-section="products"]{position:relative!important;display:flex!important;align-items:center!important;gap:0!important}
#sales-details-modal .sales-section-ear-pairs{position:absolute;left:50%;top:50%;display:inline-flex;align-items:center;gap:18px;margin:0;transform:translate(-50%,-50%)}
#sales-details-modal .sales-section-ear-pair{display:inline-flex!important;align-items:center;justify-content:center;gap:5px;box-sizing:border-box;min-width:43px!important;height:21px!important;min-height:21px!important;max-height:21px!important;margin:0!important;padding:1px 4px!important;border:1px solid transparent;border-radius:3px;background:transparent;cursor:pointer}
#sales-details-modal .sales-section-ear-pair[aria-pressed="true"]{border-color:#6e9bc1;background:#e7f4ff;box-shadow:inset 0 0 0 1px #fff}
#sales-details-modal .sales-section-ear-pair:hover{border-color:#8bb1d0}
#sales-details-modal .sales-section-ear-pair:focus-visible{outline:2px solid #2476b9;outline-offset:1px}
#sales-details-modal .sales-section-ear-pair .ti{width:16px!important;margin:0!important;font-size:16px;line-height:1;color:#159447!important}
#sales-details-modal .sales-section-ear-pair .sales-ear-left{transform:scaleX(-1)}
#sales-details-modal .sales-section-ear-pair .sales-ear-red{color:#d83b40!important}
#sales-details-modal #hearing-device-details .sales-same-model-field{display:none!important}
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details-2{display:none!important}
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details>label:has([name="sales_device_sgk"]),
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details>label:has([name="sales_device_net_price"]),
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details>label:has([data-list-price-for="sales_device_serial"]){display:none!important}
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details .sales-same-model-field{display:flex!important;flex-direction:column;gap:5px}
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details .sales-same-model-list-price{line-height:16px!important}
#sales-details-modal[data-hearing-pair="same"] #hearing-device-details .sales-same-model-field input{width:100%;box-sizing:border-box}
...
#sales-details-modal #charger-device-details{grid-template-columns:repeat(5,minmax(0,1fr))!important}
#sales-details-modal #charger-device-details>label.charger-promotion-field{grid-column:1/-1!important;justify-self:center!important;width:180px!important;max-width:100%!important;margin:16px 0 6px!important}
...
(()=>{
  const setup=()=>{
    ...
    const decorateSections=()=>{
      ...
      if(productTitle&&!productTitle.querySelector('.sales-section-ear-pairs')){
        const ears=document.createElement('span');ears.className='sales-section-ear-pairs';ears.setAttribute('role','group');ears.setAttribute('aria-label','İşitme cihazı eşleşmesi');
        ears.innerHTML='<button type="button" class="sales-section-ear-pair" data-hearing-pair="same" aria-label="Aynı marka ve modelden iki işitme cihazı" title="Aynı marka ve modelden 2 cihaz"><i class="icon-base ti tabler-ear" aria-hidden="true"></i><i class="icon-base ti tabler-ear sales-ear-left" aria-hidden="true"></i></button><button type="button" class="sales-section-ear-pair" data-hearing-pair="separate" aria-label="İki farklı model işitme cihazı" title="İki farklı model"><i class="icon-base ti tabler-ear" aria-hidden="true"></i><i class="icon-base ti tabler-ear sales-ear-left sales-ear-red" aria-hidden="true"></i></button>';
        productTitle.append(ears);
      }
      productTitle?.querySelectorAll('[data-hearing-pair]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.hearingPair===(modal.dataset.hearingPair||'separate'))));
...
/* Each payment uses the full dialog instead of a separately scrolling slice. */
.classic-cash-window-modal>.classic-cash-window-dialog.payment-tabs-dialog{height:auto!important;max-height:calc(100dvh - 20px)!important;overflow:auto!important}
.classic-cash-window-dialog.payment-tabs-dialog>section{flex:0 0 auto!important;max-height:none!important;overflow:visible!important;background:var(--payment-color)!important}
.classic-cash-window-dialog.payment-tabs-dialog>section>strong{display:none!important}
.classic-cash-window-dialog [data-company-payment-account]{display:none!important}
.classic-cash-window-dialog header [data-income-header-total],.classic-cash-window-dialog header [data-income-header-total] span{color:#ff4d4f!important}
.payment-tabs-dialog>[data-payment-tabs]{display:flex;flex:0 0 auto;gap:4px;padding:6px;background:#dcebf8}
.payment-tabs-dialog>[data-payment-tabs]>button{flex:1;min-width:0;min-height:29px;padding:4px;border:1px solid #8daece;border-radius:3px;background:var(--payment-color);color:#26384a;font:700 11px Tahoma,"Segoe UI",sans-serif;cursor:pointer}
.payment-tabs-dialog>[data-payment-tabs]>button[aria-selected=true]{border:2px solid #176c5b;box-shadow:inset 0 -3px #176c5b}
...
.classic-cash-window-dialog>section>label{display:flex;flex-direction:column!important;min-width:0!important;gap:2px!important;margin:0!important;padding:0!important;border:0!important;color:#26384a!important;font:700 11px/15px Tahoma,"Segoe UI",sans-serif!important}
.classic-cash-window-dialog :is([data-primary-term-schedule],[data-term-schedule],[data-stage-schedule]),
.classic-cash-window-dialog :is([data-primary-term-schedule],[data-term-schedule],[data-stage-schedule]) :is(label,span,strong,div){font-family:Tahoma,"Segoe UI",sans-serif!important;font-size:11px!important;line-height:15px!important}
...
.classic-cash-window-dialog>footer .classic-cash-save .tabler-device-floppy{display:inline-block!important;flex:0 0 16px!important;width:16px!important;height:16px!important;font-size:16px!important;line-height:16px!important}
...
(()=>{
  const setup=()=>{
    ...
    const normalizeFooter=()=>{
      ...
      const save=[...footer.querySelectorAll('button')].find(button=>!button.matches('.repair-cancel,[data-cash-close],[data-cancel-last-income],[aria-label="Bir gelir kaydı daha ekle"]'));
      if(save){save.classList.add('classic-cash-save');if(save.textContent.trim()!=='Kaydet (F2)'||!save.querySelector('.tabler-device-floppy'))save.innerHTML='<i class="ti tabler-device-floppy" aria-hidden="true"></i> Kaydet (F2)';if(save.type==='submit')save.title='Kaydet — F2';save.setAttribute('aria-label','Kaydet');save.style.setProperty('display','inline-flex','important');save.style.setProperty('width','auto','important');save.style.setProperty('min-width','114px','important');save.style.setProperty('max-width','none','important');['height','min-height','max-height'].forEach(property=>save.style.setProperty(property,'29px','important'));save.style.setProperty('padding','0 10px','important');}
    };
    normalizeFooter();
