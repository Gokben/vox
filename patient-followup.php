...
const initializeSalesScreen=()=>{
  ...
  const salesTitle=detailsModal?.querySelector('#sales-details-title'),salesGrandTotal=(()=>{if(!salesTitle)return null;let output=salesTitle.querySelector('#sales-grand-total');if(output)return output;output=document.createElement('span');output.id='sales-grand-total';output.textContent='Toplam Satış: 0,00 ₺';output.style.cssText='color:#e0444c;font-weight:700';salesTitle.append(output);salesTitle.style.cssText+=';display:flex;align-items:center;gap:18px';return output;})();
...
<style>#sales-details-link[hidden]{display:none!important}#sales-lock-toggle{order:-1}</style>
