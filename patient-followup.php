...
$salesExitSerialStatement = $pdo->prepare("SELECT s.id,s.stock_type,s.brand,s.model,m.invoice_no,m.serial_numbers FROM stock_movements m INNER JOIN stock_cards s ON s.id=m.stock_id WHERE s.stock_type IN (?,?) AND m.movement_type='Çıkış' AND COALESCE(m.invoice_no,'')<>'' AND COALESCE(m.serial_numbers,'') NOT IN ('','[]') ORDER BY m.id DESC");
$salesExitSerialStatement->execute(['İşitme Cihazı','Şarj Cihazı']);
...
const initializeSalesScreen=()=>{
  ...
  const invoiceMatchedSerials=(brand,model,type='İşitme Cihazı')=>{const invoice=(salesInvoiceInput?.value||'').trim();return invoice?salesExitSerials.filter(item=>item.stock_type===type&&item.brand===brand&&item.model===model&&String(item.invoice_no||'').trim()===invoice):[];};
