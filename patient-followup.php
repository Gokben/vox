...
<script>
(() => {
  const form=document.getElementById('service-card-form'), service=form?.querySelector('[name="service_name"]'), modal=document.getElementById('sales-stock-modal'), value=document.getElementById('sales_stock_id'), search=document.getElementById('sales-stock-search');
  ...
  service.addEventListener('change',()=>{if(isSales())open();else{value.value='';close()}});
  modal.querySelectorAll('[data-sales-close]').forEach(x=>x.addEventListener('click',close));items.forEach(item=>item.addEventListener('click',()=>{value.value=item.dataset.id||'';close()}));search?.addEventListener('input',()=>{const q=search.value.trim().toLocaleLowerCase('tr-TR');items.forEach(item=>item.hidden=!!q&&!(item.dataset.search||'').includes(q))});
})();
</script>
<script>
(() => {
