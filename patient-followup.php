...
const initializeSalesScreen=()=>{
  ...
  const syncTotalDiscountMode=()=>{const enabled=hasSecondHearingDevice(),label=totalDiscountInput?.closest('label');if(label)label.hidden=!enabled;detailsModal?.querySelectorAll('[name="sales_device_discount_rate"],[name="sales_device_2_discount_rate"],[name="sales_device_3_discount_rate"],[name="sales_device_4_discount_rate"]').forEach(field=>{const fieldLabel=field.closest('label');if(fieldLabel){fieldLabel.hidden=enabled;fieldLabel.style.display=enabled?'none':'flex';}});};
