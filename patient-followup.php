...
const initializeSalesScreen=()=>{
  ...
  totalDiscountInput?.addEventListener('input',()=>{
    ...
    const listTotal=devices.reduce((sum,device)=>sum+device.list,0);
    ...
    const requested=raw.includes('%')?listTotal*entered/100:entered;
  ...
  refreshSameModelSummary=()=>{
    ...
    const net=['sales_device_net_price','sales_device_2_net_price'].reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);
    ...
    sameNetTotal.value=formatTurkishMoney(net);
