...
const initializeSalesScreen=()=>{
  ...
  const salesFooter=detailsModal?.querySelector('.repair-dialog>footer'),salesGrandTotal=(()=>{if(!salesFooter)return null;let output=detailsModal.querySelector('#sales-grand-total');if(output)return output;output=document.createElement('span');output.id='sales-grand-total';output.textContent='Toplam Satış: 0,00 ₺';output.style.cssText='margin-right:auto;color:#e0444c;font-weight:700';salesFooter.prepend(output);return output;})();
