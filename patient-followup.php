...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedServiceName === 'Satış') {
        try {
            $consumableLines = sale_consumable_lines(json_decode((string)$values['sales_details'], true) ?: []);
...
const initializeSalesScreen=()=>{
  ...
  const fillChargerSerial=()=>{const stock=chargerDeviceStocks.find(item=>item.brand===(chargerBrandSelect?.value||'')&&item.model===(chargerModelSelect?.value||''));setListPriceHint([chargerBrandSelect,chargerModelSelect,chargerSerialInput],stock);if(chargerPriceInput)chargerPriceInput.value=stock?(listPriceForStock(stock)||formatTurkishMoney(stock.sale_price||0)):'';applyDiscount(chargerPriceInput,chargerDiscountInput,chargerNetPriceInput);if(!stock||!chargerSerialInput)return;try{const serials=JSON.parse(stock.serial_numbers||'[]');const serial=Array.isArray(serials)?serials.find(value=>String(value).trim()!==''):'';if(serial)chargerSerialInput.value=serial;}catch(_){}};
