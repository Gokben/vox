...
const initializeSalesScreen=()=>{
  ...
  const salesFooter=detailsModal?.querySelector('.repair-dialog>footer'),salesGrandTotal=(()=>{if(!salesFooter)return null;let output=detailsModal.querySelector('#sales-grand-total');if(output)return output;output=document.createElement('span');output.id='sales-grand-total';output.textContent='Toplam Satış: 0,00 ₺';output.style.cssText='color:#e0444c;font-weight:700';salesFooter.prepend(output);if(salesLockButton)salesFooter.insertBefore(salesLockButton,salesFooter.querySelector('[data-sales-details-close]'));return output;})();
  ...
  const salesCashForm=document.querySelector('form[action*="cash.php"]');if(salesCashForm){if(!salesCashForm.querySelector('[name="source_url"]')){const source=document.createElement('input');source.type='hidden';source.name='source_url';source.value=<?=json_encode(url('patient-followup.php?id='.$id))?>;salesCashForm.append(source);}const payment=salesCashForm.querySelector('[name="payment_type"]');if(payment&&!salesCashForm.querySelector('[data-sales-cash-current-account]')){const accountLabel=document.createElement('label'),accountSelect=document.createElement('select');accountLabel.dataset.salesCashCurrentAccount='1';accountLabel.textContent='Cari Hesap';accountLabel.style.cssText='display:none;grid-column:1/-1;flex-direction:column;gap:5px';accountSelect.name='current_account_id';accountSelect.append(new Option('Cari hesap seçiniz',''));const accounts=<?=json_encode($mailOrderAccounts, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>;accounts.forEach(account=>accountSelect.add(new Option('['+account.code+'] '+(account.short_name||account.title),String(account.id))));accountLabel.append(accountSelect);payment.closest('label')?.after(accountLabel);const syncCurrentAccount=()=>{const mailOrder=payment.value==='mail_order';accountLabel.style.display=mailOrder?'flex':'none';accountSelect.required=mailOrder;};payment.addEventListener('change',syncCurrentAccount);document.addEventListener('click',event=>{if(event.target.closest('[aria-label="Kasa"]'))setTimeout(syncCurrentAccount,0);});syncCurrentAccount();}}
...
<style>#sales-details-link[hidden]{display:none!important}#sales-lock-toggle{width:44px!important;min-width:44px!important;height:44px!important;min-height:44px!important;margin-right:auto!important;font-size:20px!important}</style>
