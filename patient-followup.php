...
const initializeSalesScreen=()=>{
  ...
  const hasSecondHearingDevice=()=>!!detailsModal?.querySelector('#hearing-device-details-2');
  const syncTotalDiscountMode=()=>{const enabled=hasSecondHearingDevice(),label=totalDiscountInput?.closest('label');if(label){label.hidden=!enabled;label.style.cssText=enabled?'display:flex!important;flex-direction:column;gap:7px':'display:none!important';}detailsModal?.querySelectorAll('[name="sales_device_discount_rate"],[name="sales_device_2_discount_rate"],[name="sales_device_3_discount_rate"],[name="sales_device_4_discount_rate"]').forEach(field=>{const fieldLabel=field.closest('label');if(fieldLabel){fieldLabel.hidden=false;fieldLabel.style.cssText=enabled?'display:flex!important;visibility:hidden;flex-direction:column;gap:7px':'display:flex!important;visibility:visible;flex-direction:column;gap:7px';}});};
  const updateTotalAmount=()=>{const totalField=detailsModal?.querySelector('[name="sales_payment_amount"]');if(!totalField)return;syncTotalDiscountMode();const netFields=['sales_device_net_price','sales_device_2_net_price','sales_device_3_net_price','sales_device_4_net_price','sales_charger_net_price'],sgkFields=['sales_device_sgk','sales_device_2_sgk','sales_device_3_sgk','sales_device_4_sgk','sales_charger_sgk'];let total=netFields.reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);const totalSgk=sgkFields.reduce((sum,name)=>sum+(parseTurkishMoney(detailsModal?.querySelector(`[name="${name}"]`)?.value)||0),0);if(totalSgkInput)totalSgkInput.value=totalSgk>0?formatTurkishMoney(totalSgk):'';const consumablePrice=parseTurkishMoney(detailsModal?.querySelector('[name="sales_consumable_price"]')?.value)||0,consumableQuantity=Number(detailsModal?.querySelector('[name="sales_consumable_quantity"]')?.value)||0;total+=consumablePrice*consumableQuantity;if(hasSecondHearingDevice()){const raw=totalDiscountInput?.value.trim()||'',discount=parseTurkishMoney(raw);if(discount!==null&&raw!=='')total=Math.max(0,raw.includes('%')?total*(1-discount/100):total-discount);}totalField.value=total>0?formatTurkishMoney(total):'';const paymentType=detailsModal?.querySelector('[name="sales_payment_type"]'),paymentLocked=<?=json_encode($savedCashRecord !== [])?>;if(paymentType){if(total<=0)paymentType.value='';paymentType.disabled=paymentLocked||total<=0;paymentType.title=paymentLocked?'Gelir kaydı bulunduğu için ödeme şekli değiştirilemez.':(total<=0?'Ürün ve toplam tutar olmadan ödeme şekli seçilemez.':'');}};
  ...
  setTimeout(()=>{try{const saved=JSON.parse(details?.value||'{}');for(let number=1;number<=4;number++){const isFirst=number===1,device=isFirst?deviceDetails:detailsModal?.querySelector(`#hearing-device-details-${number}`),brand=isFirst?brandSelect:device?.querySelector(`[name="sales_device_${number}_brand"]`),model=isFirst?modelSelect:device?.querySelector(`[name="sales_device_${number}_model"]`),serial=isFirst?deviceSerialInput:device?.querySelector(`[name="sales_device_${number}_serial"]`),sgk=isFirst?detailsModal?.querySelector('[name="sales_device_sgk"]'):device?.querySelector(`[name="sales_device_${number}_sgk"]`),savedBrand=saved[isFirst?'sales_brand':`sales_device_${number}_brand`]||'',savedModel=saved[isFirst?'sales_model':`sales_device_${number}_model`]||'',savedSerial=saved[isFirst?'sales_device_serial':`sales_device_${number}_serial`]||'';if(!device||!brand||!model||!serial||!savedBrand||!savedModel)continue;brand.value=savedBrand;if(![...model.options].some(option=>option.value===savedModel))model.add(new Option(savedModel,savedModel));model.disabled=false;model.value=savedModel;const stocks=hearingDeviceStocks.filter(item=>item.brand===savedBrand&&item.model===savedModel);fillSerialOptions(serial,[...stocks,...invoiceMatchedSerials(savedBrand,savedModel)]);if(savedSerial&&![...serial.options].some(option=>option.value===savedSerial))serial.add(new Option(savedSerial,savedSerial));serial.value=savedSerial;if(sgk)sgk.value=saved[isFirst?'sales_device_sgk':`sales_device_${number}_sgk`]||sgk.value;}}catch(_){}},0);
  [80,250,600].forEach(delay=>setTimeout(()=>{try{const saved=JSON.parse(details?.value||'{}');[[deviceSerialInput,saved.sales_device_serial],...[2,3,4].map(number=>[detailsModal?.querySelector(`[name="sales_device_${number}_serial"]`),saved[`sales_device_${number}_serial`]])].forEach(([serial,savedSerial])=>{if(!serial||!savedSerial)return;if(![...serial.options].some(option=>option.value===savedSerial))serial.add(new Option(savedSerial,savedSerial));serial.value=savedSerial;});}catch(_){}updateTotalAmount();},delay));
