...
const initializeSalesScreen=()=>{
  ...
  setTimeout(()=>{try{const saved=JSON.parse(details?.value||'{}');for(let number=2;number<=4;number++){const device=detailsModal?.querySelector(`#hearing-device-details-${number}`),brand=device?.querySelector(`[name="sales_device_${number}_brand"]`),model=device?.querySelector(`[name="sales_device_${number}_model"]`),serial=device?.querySelector(`[name="sales_device_${number}_serial"]`),sgk=device?.querySelector(`[name="sales_device_${number}_sgk"]`),savedBrand=saved[`sales_device_${number}_brand`]||'',savedModel=saved[`sales_device_${number}_model`]||'',savedSerial=saved[`sales_device_${number}_serial`]||'';if(!device||!brand||!model||!serial||!savedBrand||!savedModel)continue;brand.value=savedBrand;if(![...model.options].some(option=>option.value===savedModel))model.add(new Option(savedModel,savedModel));model.disabled=false;model.value=savedModel;const stocks=hearingDeviceStocks.filter(item=>item.brand===savedBrand&&item.model===savedModel);fillSerialOptions(serial,[...stocks,...invoiceMatchedSerials(savedBrand,savedModel)]);if(savedSerial&&![...serial.options].some(option=>option.value===savedSerial))serial.add(new Option(savedSerial,savedSerial));serial.value=savedSerial;if(sgk)sgk.value=saved[`sales_device_${number}_sgk`]||sgk.value;}}catch(_){}},0);
