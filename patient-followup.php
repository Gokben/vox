...
<?php endif; ?>
<script>(()=>{const removeLabelText=selector=>{const label=document.querySelector(selector)?.closest('label');if(!label)return;label.setAttribute('aria-label',selector.includes('repair_target')?'Hedef':'Teknik servis');[...label.childNodes].filter(node=>node.nodeType===Node.TEXT_NODE).forEach(node=>node.remove());};removeLabelText('[name="repair_target"]');removeLabelText('[name="repair_technician"]');})();</script>
<?php if($showForm): ?>
