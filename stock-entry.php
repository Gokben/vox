...
if (!isset($_GET['new'])) {
    ...
    if ($filterText !== '') {
        ...
        $like = '%' . $filterText . '%';
        array_push($filterParams, $like, $like, $like, $like, $like, $like);
    }
    ...
    $summaryStatement = $pdo->prepare('SELECT COUNT(DISTINCT CASE WHEN TRIM(COALESCE(m.invoice_no,""))<>"" THEN LOWER(TRIM(m.invoice_no)) END) AS invoice_count,COALESCE(SUM(COALESCE(m.purchase_price,0)*COALESCE(m.quantity,0)),0) AS amount_excluding_vat,COALESCE(SUM(COALESCE(m.purchase_price,0)*COALESCE(m.quantity,0)*(1+COALESCE(m.vat_rate,0)/100.0)),0) AS amount_including_vat FROM stock_movements m INNER JOIN stock_cards s ON s.id=m.stock_id LEFT JOIN current_accounts c ON c.id=m.current_account_id WHERE ' . $filterSql);
...
<style>.bulk-stock-entry{position:absolute;right:190px;top:50%;transform:translateY(-50%);text-decoration:none}.invoice-stock-entry{position:absolute;right:365px;top:50%;transform:translateY(-50%);text-decoration:none}@media(max-width:720px){.bulk-stock-entry,.invoice-stock-entry{position:static!important;transform:none!important;display:block;margin:12px 24px 0;text-align:center}}</style>
