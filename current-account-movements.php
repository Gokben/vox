    ...
    <div><table>
      ...
      <?php foreach ($groupedRows as $index => $row): ?>
        <tr>
          ...
          <td class="<?= $row['movement_type'] === 'Giriş' ? 'total-price-entry' : '' ?>"><?=e(number_format($row['total_price'], 2, ',', '.'))?> TL</td><td class="<?= $row['movement_type'] === 'Giriş' ? 'total-price-entry' : '' ?>"><?=e(number_format($row['total_price_with_vat'], 2, ',', '.'))?> TL</td><td><input class="invoice-gross-value" type="text" value="<?=e(number_format($row['gross_amount'], 2, ',', '.'))?>" aria-label="İskontosuz fatura tutarı" readonly></td><td><?=e(number_format($row['gross_amount'] > 0 ? $row['gross_amount'] - $row['total_price'] : 0, 2, ',', '.'))?> TL</td><td><?=e(number_format($row['gross_amount'] > 0 ? (($row['gross_amount'] - $row['total_price']) / $row['gross_amount']) * 100 : 0, 2, ',', '.'))?>%</td>
        ...
        <tr class="invoice-detail-row" data-invoice-detail="<?=$index?>" hidden><td colspan="11"><table class="invoice-items"><thead><tr><th>STOK KARTI</th><th>MİKTAR</th><th>İSKONTO ORANI<?php if (preg_replace('/\D/', '', (string)($account['tax_number'] ?? '')) === '3210362123' && $row['movement_type'] === 'Giriş' && $row['invoice_no'] !== '' && $row['gross_amount'] > 0): ?> <span class="signia-effective-discount" title="Fatura geneli iskonto: (iskontosuz tutar − iskonto sonrası tutar) / iskontosuz tutar × 100. Bilgi amaçlıdır; kalem oranlarını değiştirmez."><?=e('%'.number_format((($row['gross_amount'] - $row['total_price']) / $row['gross_amount']) * 100, 4, ',', '.'))?></span><?php endif ?></th><th>FİYAT</th></tr></thead><tbody><?php foreach ($row['items'] as $item): ?><tr><td><?=e($item['stock'])?></td><td><?=e((string)$item['quantity'])?></td><td><form method="post" class="item-discount-inline"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="save_item_discount"><input type="hidden" name="movement_id" value="<?=e((string)$item['id'])?>"><input name="discount_rate" type="number" min="0" max="100" step="0.01" value="<?=e((string)$item['discount_rate'])?>"><span>%</span></form></td><td><?=e(number_format($item['price'], 2, ',', '.'))?> TL</td></tr><?php endforeach ?></tbody></table></td></tr>
...
<style>.invoice-items .signia-effective-discount{display:inline-block;margin-left:8px;padding:1px 5px;border:1px solid #8cb7a9;border-radius:3px;background:#e4f5ed;color:#12604d;font-weight:700;white-space:nowrap;cursor:help}</style>
