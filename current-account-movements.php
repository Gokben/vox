...
function current_movement_parse_amount(string $value): float { $value = trim(str_replace(' ', '', $value)); if (str_contains($value, ',')) $value = str_replace(',', '.', str_replace('.', '', $value)); return (float)$value; }
...
<main class="patient-container cam-page">
  <section class="cam-card">
    <header><h1>Cari Hareketleri</h1><p><?=e($account['code'].' — '.$account['title'])?></p></header>
    <div><table>
      <thead><tr><th>TARİH</th><th>HAREKET</th><th>FATURA NO</th><th>MİKTAR</th><th>T. TUTAR</th><th>T. TUTAR (KDV'Lİ)</th><th>İSKONTOSUZ FATURA TUTARI</th><th>İSKONTO</th><th>ORT İSK.</th><th>İŞLEMLER</th></tr></thead>
      <tbody>
      <?php foreach ($groupedRows as $index => $row): ?>
        <tr>
          <td><?=e(format_date_tr($row['date']))?></td><td><?=e($row['movement_type'])?></td><td><?=e($row['invoice_no'] ?: '—')?></td><td><?=e((string)$row['quantity'])?></td>
...
<style>.cam-page{max-width:1500px!important;margin:auto;padding:28px 20px}.cam-card{background:#fff;border:1px solid #e1e2e8;border-radius:10px;overflow:hidden;box-shadow:0 3px 12px #1e283c0f}.cam-card header{padding:22px 24px;border-bottom:1px solid #e1e2e8}.cam-card h1{margin:0 0 5px;font-size:21px}.cam-card p{margin:0;color:#7b7b8d}.cam-card>div{overflow:auto}.cam-card table{width:100%;min-width:1050px;border-collapse:collapse}.cam-card th,.cam-card td{padding:14px 18px;border-bottom:1px solid #e1e2e8;text-align:left}.cam-card th{font-size:12px;color:#6d6b7f}.empty{text-align:center;color:#7b7b8d}.total-price-entry{color:#e04f55;font-weight:700}.invoice-details-toggle{display:inline-grid;place-items:center;width:32px;height:32px;border:0;border-radius:6px;background:#f5a33b;color:#000;font-size:20px;font-weight:700;cursor:pointer}.invoice-detail-row td{padding:0;background:#fff8ed}.invoice-items{width:100%;min-width:0!important}.invoice-items th,.invoice-items td{padding:10px 14px}.invoice-items th{background:#fff0d9}.invoice-items th:last-child,.invoice-items td:last-child{text-align:right}</style><?php patient_footer(); ?>
