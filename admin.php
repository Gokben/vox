...
<main class="patient-container personnel-page">
 ...
 <details class="vuexy-form-card new-user-accordion"<?=$error!==''?' open':''?>><summary class="form-card-title"><span><h1>Yeni Kullanıcı</h1><p>Uygulamaya erişecek kullanıcının giriş bilgilerini ve rolünü tanımlayın.</p></span><i class="new-user-chevron" aria-hidden="true"></i></summary>
 ...
 </form><script>document.querySelector('.personnel-form select[name="role"]').innerHTML='<option value="company_manager">Firma Yöneticisi</option><option value="audiometrist">Odyometrist</option><option value="secretary" selected>Sekreter</option><option value="accounting">Muhasebe</option>';</script></details>
...
<style>
.new-user-accordion>summary{position:relative;display:flex!important;align-items:center;justify-content:space-between;gap:20px;padding-right:64px;cursor:pointer;list-style:none;user-select:none}
.new-user-accordion>summary::-webkit-details-marker{display:none}
.new-user-accordion>summary::marker{display:none;content:""}
.new-user-accordion .new-user-chevron{position:absolute;right:24px;top:50%;display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;margin:0;font-style:normal;font-size:20px;font-weight:500;line-height:1;transform:translateY(-50%);transition:transform .2s ease}
.new-user-accordion .new-user-chevron::before{content:">"}
.new-user-accordion[open] .new-user-chevron{transform:translateY(-50%) rotate(90deg)}
.new-user-accordion:not([open])>summary{border-bottom:0}
</style>
<script>(()=>{const tabs=document.querySelector('.settings-tabs');if(tabs){const link=document.createElement('a');link.href='<?=url('social-securities.php')?>';link.textContent='Sosyal Güvence';tabs.appendChild(link)}})();</script>
<?php patient_footer();?>
