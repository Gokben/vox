...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($fields as $field) {
            if ($field === 'related_cards') {
                $selectedIds = is_array($_POST['related_cards'] ?? null) ? $_POST['related_cards'] : [];
                $selectedIds = array_values(array_unique(array_filter(array_map('intval', $selectedIds), static fn(int $selectedId): bool => $selectedId > 0 && $selectedId !== $id)));
                $unit[$field] = json_encode($selectedIds, JSON_UNESCAPED_UNICODE);
                continue;
            }
...
$selectedRelatedCards = json_decode((string)($unit['related_cards'] ?? ''), true);
if (!is_array($selectedRelatedCards)) $selectedRelatedCards = [];
$selectedRelatedCards = array_map('intval', $selectedRelatedCards);
patient_header($editId ? 'Ünite Düzenle' : ($showForm ? 'Yeni Ünite' : 'Pazarlama'), 'cash');
?>
<style>
...
<link rel="stylesheet" href="<?=url('assets/classic-units.css?v=20260824-3')?>">
<style>
  .unit-related-card-options{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;flex:1;min-width:0}
  .unit-related-card-option{display:flex;align-items:center;gap:8px;min-height:38px;padding:8px 10px;border:1px solid #d5d3de;border-radius:6px;background:#fff;cursor:pointer}
  .unit-related-card-option:has(input:checked){border-color:#20a447;background:#effaf2}
