...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_print_settings') {
        ...
        if ($name === '') $error = 'Soru alanı zorunludur.';
            ...
            elseif ($id) { $pdo->prepare('UPDATE anamnesis_question_definitions SET name=?,active=?,sort_order=? WHERE id=?')->execute([$name,$active,$sort,$id]); $message='Anamnez sorusu güncellendi.'; }
            else { $pdo->prepare('INSERT INTO anamnesis_question_definitions(name,active,sort_order) VALUES(?,?,?)')->execute([$name,$active,$sort]); $message='Anamnez sorusu eklendi.'; }
        }
    }
}
