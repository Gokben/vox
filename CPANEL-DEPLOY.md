...
Hedef adres: `https://voxisitme.com/erp`

## Kod dağıtımı

`.cpanel.yml` uygulama dosyalarını `$HOME/public_html/erp/` klasörüne kopyalar. CRM ile ilgisi olmayan `voxweb` klasörü dağıtıma dahil edilmez.
...
`public_html/erp/config.local.php` dosyasını sunucuda oluşturun. Bu dosya Git tarafından takip edilmez:
...
4. Eski sunucudaki `assets/uploads/` içeriğini yeni sunucuda `public_html/erp/assets/uploads/` altına kopyalayın.
...
- `public_html/erp/assets/uploads/` için PHP yazma izni
